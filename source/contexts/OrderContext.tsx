...
interface OrderContextType {
  ...
  // Ações de Pedidos
  addOrder: (
    order: Omit<Order, "id" | "time" | "createdAt" | "messages">,
  ) => void;
...
export function OrderProvider({ children }: { children: ReactNode }) {
  ...
  const addOrder = useCallback(
    (newOrderData: Omit<Order, "id" | "time" | "createdAt" | "messages">) => {
      ...
      setOrders((prev) =>
        prev.map((o) => {
          if (o.id === orderId) {
            ...
            let systemMsg = "";
            if (status === "preparing")
              systemMsg = "Pedido aceito e em preparo.";
            if (status === "ready") systemMsg = "Pedido pronto!";
            if (status === "delivering")
              systemMsg = "Pedido saiu para entrega.";
  ...
  const getOrdersByService = useCallback(
    (service: ServiceType) => {
