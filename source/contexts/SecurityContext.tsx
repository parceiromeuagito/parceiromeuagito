...
export type Permission =
    ...
    | 'pos:view' | 'pos:discount' | 'pos:open' | 'pos:close'
...
export const SecurityProvider = ({ children }: { children: ReactNode }) => {
    ...
    useEffect(() => {
        ...
        if (stored) {
            try {
                ...
                const validUser = members.find(m => m.id === storedUser.id && m.active);
                if (validUser) {
                    setCurrentUser(validUser);
                    console.log(`🔐 Sessão restaurada: ${validUser.name} (${validUser.role})`);
                } else {
    ...
    const handleGateSuccess = (user: TeamMember) => {
        ...
        if (hasRequiredPermission) {
            gateState.callback?.();
            setGateState({ isOpen: false, permission: 'dashboard:view', callback: null });
        } else {
            alert(`Acesso Negado: O perfil "${user.role}" não tem permissão para esta ação.`);
            setGateState({ isOpen: false, permission: 'dashboard:view', callback: null });
        }
    };
    ...
    return (
        <SecurityContext.Provider value={{
            ...
            <SecurityGateModal
                ...
                onClose={() => setGateState({ isOpen: false, permission: 'dashboard:view', callback: null })}
