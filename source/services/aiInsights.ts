...
const EXTERNAL_CONTEXT = {
  ...
  events: [{ name: "Show no Estádio", daysUntil: 5, impact: "high" }],
};
...
export class AIInsightsService {
  static predictDemand(historicalData: number[]): {
    ...
    const trend =
      slope > avg * 0.05 ? "up" : slope < -(avg * 0.05) ? "down" : "stable";
  ...
  static generateOptimizations(orders: Order[]): string[] {
    ...
    const peakHourEntry = Object.entries(hourlyOrders).sort(
      ([, a], [, b]) => b - a,
    )[0];
    ...
    if (peakHourEntry) {
      ...
      suggestions.push(
        `Horário de pico detectado às ${peakHour}h. Considere reforçar a equipe neste horário.`,
      );
    }
    ...
    if (avgTicket < 50) {
      suggestions.push(
        "Seu ticket médio está abaixo de R$ 50. Crie combos para aumentar o valor por pedido.",
      );
    } else if (avgTicket > 150) {
      suggestions.push(
        "Ticket médio alto! Ofereça programas de fidelidade para reter esses clientes VIP.",
      );
    }
    ...
    if (cancelledCount > orders.length * 0.1) {
      suggestions.push(
        `Taxa de cancelamento alta (${cancelledCount} pedidos). Verifique seu estoque e tempo de resposta.`,
      );
    }
