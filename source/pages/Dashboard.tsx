...
const StatCard = ({ title, value, subtext, icon: Icon, trend }: StatCardProps) => (
  ...
  >
    <div className="flex items-start justify-between mb-4">
      ...
      {trend && (
        <span className={`flex items-center text-xs font-bold px-2 py-1 rounded-full ${Number(trend) > 0 ? 'text-green-400 bg-green-900/20' : 'text-red-400 bg-red-900/20'}`}>
...
const Dashboard = () => {
  ...
  const stats = useMemo(() => {
    ...
    const active = orders.filter(o => ['pending', 'preparing', 'accepted', 'ready', 'delivering'].includes(o.status)).length;
  ...
  const MainIcon = useMemo(() => {
    return config.businessTypes.length > 1 ? Layers :
      config.primaryType === 'hotel' ? BedDouble :
        config.primaryType === 'scheduling' ? CalendarCheck :
          config.primaryType === 'tickets' ? Ticket :
            config.primaryType === 'reservation' ? UtensilsCrossed :
              config.primaryType === 'delivery' ? Truck :
                Users;
    ...
    series: [
      {
        ...
        showSymbol: false,
        data: [1200, 1320, 1010, 1340, 2900, 3300, stats.revenue > 4000 ? stats.revenue : 3100]
      }
    ]
  }), [stats.revenue]);
    ...
    <div className="space-y-8 animate-in fade-in duration-500 pb-10">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-zinc-400">Acompanhe o desempenho do seu {context.label.toLowerCase()}.</p>
        </div>
        ...
        <div className="flex gap-3">
          ...
          <PremiumFeature minPlan="pro" fallback={
            <button className="flex items-center gap-2 px-4 py-2 bg-zinc-800 text-zinc-500 rounded-lg text-sm font-medium cursor-not-allowed" title="Requer Plano Pro">
            ...
            <div className="flex items-center justify-between mb-8">
              ...
              <div className="flex gap-2">
                {['Dia', 'Semana', 'Mês'].map(period => (
                  <button key={period} className="px-3 py-1 text-xs font-medium rounded-lg hover:bg-zinc-800 text-zinc-400 transition-colors">
                    {period}
                  </button>
                ))}
              </div>
