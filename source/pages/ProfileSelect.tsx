...
const ProfileSelect = () => {
  ...
  const availableRoles = Array.from(
    new Set(members.filter((m) => m.active).map((m) => m.role)),
  );
  ...
  const handlePinSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      const member = members.find(
        (m) => m.role === selectedRole && m.pin === pin && m.active,
      );

      if (member) {
        loginWithMember(member);
        addToast(
          `Bem-vindo, ${member.name}! Você entrou como ${getRoleInfo(member.role).title}.`,
          "success",
        );
    ...
    <div className="min-h-screen bg-gradient-to-br from-zinc-950 via-zinc-900 to-zinc-950 flex items-center justify-center p-4">
      ...
      >
          ...
          {!selectedRole ? (
              ...
              {availableRoles.map((role) => {
                ...
                return (
                  ...
                  >
                    ...
                    <ArrowRight
                      className={`w-5 h-5 ${info.textColor} opacity-0 group-hover:opacity-100 transition-opacity`}
                    />
                  </motion.button>
                );
              })}
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                <div
                  className={`w-10 h-10 rounded-xl bg-gradient-to-br ${getRoleInfo(selectedRole).color} flex items-center justify-center`}
                >
                  {React.createElement(getRoleInfo(selectedRole).icon, {
              ...
              <form onSubmit={handlePinSubmit}>
                ...
                >
                  {[0, 1, 2, 3].map((i) => (
                    ...
                    >
                      {pin.length > i && (
                        <span className={getRoleInfo(selectedRole).textColor}>
                          ●
                        </span>
                      )}
                    </div>
                ...
                <div className="grid grid-cols-3 gap-2 mb-4">
                  ...
                  <button
                    ...
                    className={`h-14 rounded-xl flex items-center justify-center text-white font-bold transition-all ${
        ...
        <p className="text-center text-xs text-zinc-500 mt-6">
          Configure usuários em{" "}
          <span className="text-zinc-400">Configurações → Equipe</span>
        </p>
      </motion.div>
