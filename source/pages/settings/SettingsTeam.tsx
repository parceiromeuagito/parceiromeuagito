...
import { Plus, Trash2, Shield, User, UserCog, KeyRound, Check, X, Eye, Edit, AlertCircle } from 'lucide-react';
import { useSecurity, DEFAULT_ROLE_PERMISSIONS, Permission } from '../../contexts/SecurityContext';
...
const SettingsTeam = () => {
    ...
    const canManageTeam = hasPermission('team:edit') || currentUser?.role === 'admin';
    ...
    const getRoleBadge = (role: UserRole) => {
        switch (role) {
            case 'admin': return <span className="px-2 py-1 bg-red-500/20 text-red-400 rounded text-xs font-bold uppercase border border-red-500/30">Admin</span>;
            case 'manager': return <span className="px-2 py-1 bg-purple-500/20 text-purple-400 rounded text-xs font-bold uppercase border border-purple-500/30">Gerente</span>;
            case 'cashier': return <span className="px-2 py-1 bg-blue-500/20 text-blue-400 rounded text-xs font-bold uppercase border border-blue-500/30">Caixa</span>;
        }
    };
    ...
    const getRoleDescription = (role: UserRole) => {
        switch (role) {
            ...
            case 'manager': return 'Gerencia operações diárias';
            case 'cashier': return 'Operações de venda básicas';
        }
    };
        ...
        <div className="space-y-6">
            ...
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-sm text-zinc-400">Gerencie quem tem acesso ao sistema.</p>
                </div>
            ...
            {!canManageTeam && (
                <div className="flex items-center gap-2 px-4 py-3 bg-yellow-500/10 border border-yellow-500/30 rounded-lg text-yellow-400 text-sm">
                    ...
                    <span>Você está visualizando a equipe. Apenas administradores podem fazer alterações.</span>
                </div>
            )}
            ...
            {showPermissions && (
                <div className="bg-zinc-900 rounded-xl border border-zinc-800 overflow-hidden">
                    <div className="p-4 border-b border-zinc-800">
                        ...
                        <p className="text-xs text-zinc-500 mt-1">Permissões padrão definidas para cada tipo de cargo</p>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="bg-zinc-800/50">
                                    <th className="text-left px-4 py-3 text-zinc-400 font-medium">Permissão</th>
                                    <th className="text-center px-4 py-3 text-red-400 font-medium">Admin</th>
                                    <th className="text-center px-4 py-3 text-purple-400 font-medium">Gerente</th>
                                    <th className="text-center px-4 py-3 text-blue-400 font-medium">Caixa</th>
                                </tr>
                            </thead>
                            <tbody>
                                {Object.entries(groupedPermissions).map(([category, permissions]) => (
                                    <React.Fragment key={category}>
                                        <tr className="bg-zinc-800/30">
                                            <td colSpan={4} className="px-4 py-2 text-xs font-bold text-zinc-500 uppercase tracking-wider">
                                                {category}
                                            </td>
                                        </tr>
                                        {permissions.map(({ key, label }) => (
                                            <tr key={key} className="border-t border-zinc-800/50 hover:bg-zinc-800/20">
            ...
            {isAdding && canManageTeam && (
                <form onSubmit={handleAddMember} className="bg-zinc-900 p-4 rounded-xl border border-zinc-800 space-y-4 animate-in slide-in-from-top-2">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-zinc-500 mt-1">{getRoleDescription(newMember.role)}</p>
                        </div>
                        <div>
                            <label className="block text-xs text-zinc-500 mb-1">PIN (4 dígitos)</label>
                            <input
                                type="text"
                                maxLength={4}
                                value={newMember.pin}
                                onChange={e => {
                                    if (/^\d*$/.test(e.target.value)) setNewMember({ ...newMember, pin: e.target.value });
                                }}
                                className="w-full bg-zinc-800 border-zinc-700 rounded-lg px-3 py-2 text-white text-sm focus:ring-primary focus:border-primary"
                                placeholder="0000"
                            />
            ...
            <div className="grid gap-4">
                {members.map((member) => (
                    <div key={member.id} className="bg-zinc-900 p-4 rounded-xl border border-zinc-800 flex items-center justify-between group hover:border-zinc-700 transition-colors">
                        <div className="flex items-center gap-4">
                            <div className="w-10 h-10 rounded-full bg-zinc-800 flex items-center justify-center text-zinc-500 border border-zinc-700">
                                {member.role === 'admin' ? <Shield className="w-5 h-5 text-red-400" /> :
                                    member.role === 'manager' ? <UserCog className="w-5 h-5 text-purple-400" /> :
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                    ...
                                    {editingId === member.id ? (
                                        <input
                                            ...
                                            onBlur={(e) => {
                                                if (e.target.value) handleUpdatePin(member.id, e.target.value);
                                        ...
                                        // autoFocus removido por acessibilidade - use focus programático se necessário
                                        />
                                    ) : (
                                        <button
                                        ...
                                        >
                                            ...
                                            PIN: ****
                                            {canManageTeam && <Edit className="w-3 h-3 opacity-0 group-hover:opacity-100" />}
                                        </button>
                                    )}
                                </div>
                            </div>
