...
const generateMessages = (count: number): Message[] => {
  return Array.from({ length: count })
    .map(() => ({
      ...
      sender: (Math.random() > 0.5 ? "customer" : "business") as
        | "customer"
        | "business",
  ...
  return Array.from({ length: count }).map(() => {
    ...
    if (type === "hotel") {
      ...
      specificData = {
        checkIn,
        checkOut: new Date(
          checkIn.getTime() + 86400000 * faker.number.int({ min: 1, max: 7 }),
        ),
        items: [
          { id: "1", name: "Suíte Master Ocean View", quantity: 1, price: 850 },
        ],
      };
      ...
      specificData = {
        items: [
          {
            id: "1",
            name: "Front Stage - Festival de Verão",
            quantity: 2,
            price: 250,
          },
        ],
    ...
    return {
      ...
      type,
      total:
        specificData.items?.reduce((acc, i) => acc + i.price * i.quantity, 0) ||
        0,
...
export const getMockOrders = (types: BusinessType[]) =>
  generateOrders(12, types);
    ...
    const typeItems = Array.from({ length: 4 }).map(() => {
      if (type === "hotel") {
        return {
          ...
          description:
            "Acomodação completa com vista para o mar, café da manhã incluso e wi-fi de alta velocidade.",
      ...
      if (type === "tickets") {
        return {
          ...
          description:
            "Acesso exclusivo com benefícios especiais para o evento principal.",
