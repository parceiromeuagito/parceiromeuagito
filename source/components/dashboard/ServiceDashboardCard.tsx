...
export function ServiceDashboardCard({ service }: { service: ServiceConfig }) {
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case "delivery":
        return <ShoppingBag className={`w-5 h-5 ${colors.icon}`} />;
      case "restaurant":
        return <Utensils className={`w-5 h-5 ${colors.icon}`} />;
      case "hotel":
        return <BedDouble className={`w-5 h-5 ${colors.icon}`} />;
      case "tickets":
        return <Ticket className={`w-5 h-5 ${colors.icon}`} />;
      case "appointments":
        return <Calendar className={`w-5 h-5 ${colors.icon}`} />;
      default:
        return <Package className={`w-5 h-5 ${colors.icon}`} />;
    }
  };

  return (
    <Card
      className={`border-l-4 ${colors.border.replace("border", "border-l")} bg-zinc-900/50 backdrop-blur-sm border-zinc-800 overflow-hidden relative group`}
    >
      <div
      ...
      <CardHeader className="pb-2 relative z-10">
        <div className="flex justify-between items-start">
          <CardTitle
            className={`flex items-center gap-2 text-lg text-zinc-100`}
          >
            <div
          ...
          {service.autoAccept && (
            <Badge
              variant="outline"
              className="bg-yellow-500/10 text-yellow-500 border-yellow-500/20 gap-1"
            >
