...
export function ServiceSelector() {
    const { services, selectedService, setSelectedService, getOrdersByService } = useOrder();
    ...
    return (
        <div className="flex items-center gap-2 overflow-x-auto pb-2 mb-6 scrollbar-hide">
            ...
            {activeServices.map(service => {
                const pendingCount = getOrdersByService(service.id).filter(o => o.status === 'pending').length;
                ...
                return (
                    <Button
                        ...
                        className={`rounded-full gap-2 ${selectedService === service.id
                    ...
                    >
                        ...
                        {service.name}
                        {pendingCount > 0 && (
                            <Badge variant="secondary" className="ml-1 h-5 px-1.5 text-[10px] bg-red-500/20 text-red-500 hover:bg-red-500/30 border-0">
                                {pendingCount}
                            </Badge>
                        )}
                    </Button>
