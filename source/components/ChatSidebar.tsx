...
import { MessageCircle, User, Clock, ChevronRight, ChevronLeft, Send } from 'lucide-react';
...
const ChatSidebar = ({ isCollapsed = false, onToggleCollapse, externalSelectedOrder }: ChatSidebarProps) => {
    ...
    const ordersWithChat = (orders || [])
        .filter(order => order?.chatHistory && Array.isArray(order.chatHistory) && order.chatHistory.length > 0)
        .sort((a, b) => {
            const lastMessageA = a.chatHistory?.[a.chatHistory.length - 1];
            const lastMessageB = b.chatHistory?.[b.chatHistory.length - 1];
            const timeA = lastMessageA?.timestamp ? (lastMessageA.timestamp instanceof Date ? lastMessageA.timestamp.getTime() : new Date(lastMessageA.timestamp).getTime()) : 0;
            const timeB = lastMessageB?.timestamp ? (lastMessageB.timestamp instanceof Date ? lastMessageB.timestamp.getTime() : new Date(lastMessageB.timestamp).getTime()) : 0;
            return timeB - timeA;
        });

    const filteredOrders = ordersWithChat.filter(order =>
                        ...
                        >
                            ...
                            >
                                ...
                                <p
                                    className={cn(
                                        'text-xs mt-1',
                                        msg.sender === 'business' ? 'text-white/70' : 'text-gray-500 dark:text-gray-400'
                                    )}
                                >
                ...
                {filteredOrders.length === 0 ? (
                    ...
                    <div className="p-2 space-y-2">
                        {filteredOrders.map((order) => {
                            const lastMessage = order.chatHistory[order.chatHistory.length - 1];
                            const unreadCount = order.chatHistory.filter(msg => msg.sender === 'customer').length;
