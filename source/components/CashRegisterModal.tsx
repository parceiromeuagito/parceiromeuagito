...
const CashRegisterModal = ({ isOpen, onClose }: CashRegisterModalProps) => {
  ...
  const { cashRegister, openRegister, closeRegister, addCashTransaction } =
  ...
  const handleAction = () => {
    ...
    switch (action) {
      ...
      case "close":
        ...
        setTimeout(() => {
          if (
            window.confirm("Deseja imprimir o relatório de fechamento agora?")
          ) {
            printCashReport(cashRegister, config.printer);
          }
        }, 500);
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          {/* Header */}
          <div
            className={`p-6 border-b border-gray-100 dark:border-zinc-800 flex justify-between items-center ${cashRegister.isOpen ? "bg-green-50 dark:bg-green-900/20" : "bg-red-50 dark:bg-red-900/20"}`}
          >
            <div>
              <h3
                className={`text-lg font-bold flex items-center gap-2 ${cashRegister.isOpen ? "text-green-800 dark:text-green-400" : "text-red-800 dark:text-red-400"}`}
              >
                {cashRegister.isOpen ? (
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={onClose}
                className="p-2 hover:bg-white/50 dark:hover:bg-zinc-800/50 rounded-full transition-colors"
              >
          ...
          <div className="flex-1 overflow-y-auto p-6 custom-scrollbar">
              ...
              <div className="flex gap-4 mt-4 text-sm">
                <div className="bg-white/10 px-3 py-1 rounded-lg">
                  ...
                  <span className="font-bold">
                    {formatCurrency(cashRegister.startAmount)}
                  </span>
                </div>
                <div className="bg-white/10 px-3 py-1 rounded-lg">
                  <span className="text-gray-400 block text-xs">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2 pt-2">
                    <button
                      ...
                      className="flex-1 py-2 bg-white dark:bg-zinc-800 border border-gray-300 dark:border-zinc-600 rounded-lg text-sm font-bold text-gray-600 dark:text-zinc-300 hover:bg-gray-50 dark:hover:bg-zinc-700"
                    >
                      Cancelar
                    </button>
                    <button
            ...
            {action === "close" && (
              ...
              >
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="font-bold text-red-900 dark:text-red-400 mb-1">
                      Confirmar Fechamento?
                    </h4>
                    <p className="text-sm text-red-700 dark:text-red-300 mb-4">
                      Isso encerrará o turno atual e gerará o relatório Z. O
                      saldo final esperado é de{" "}
                      <strong>
                        {formatCurrency(cashRegister.currentBalance)}
                      </strong>
                      .
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => setAction("view")}
                        className="px-4 py-2 bg-white dark:bg-zinc-800 border border-red-200 dark:border-red-900/30 rounded-lg text-sm font-bold text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/10"
                      >
                        Cancelar
                      </button>
                      <button
            ...
            <div>
              ...
              <div className="space-y-2">
                {cashRegister.transactions.length === 0 ? (
                  <p className="text-center text-gray-400 dark:text-zinc-500 text-sm py-4">
                  ...
                  cashRegister.transactions.map((t) => (
                    <div
                      key={t.id}
                      className="flex justify-between items-center p-3 bg-gray-50 dark:bg-zinc-800/50 rounded-lg border border-gray-100 dark:border-zinc-800"
                    >
