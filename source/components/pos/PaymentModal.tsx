...
import { X, DollarSign, CreditCard, QrCode, Banknote, CheckCircle, Trash2, Plus } from 'lucide-react';
...
const PaymentModal = ({ isOpen, onClose, onConfirm, total }: PaymentModalProps) => {
    ...
    const [currentMethod, setCurrentMethod] = useState<'credit_card' | 'debit_card' | 'pix' | 'cash'>('cash');
    ...
    const currentChange = currentMethod === 'cash' && receivedAmount
                ...
                >
                    ...
                    <div className="flex-1 flex flex-col p-6 border-r border-gray-100 dark:border-zinc-800">
                        <div className="mb-6">
                            ...
                            <p className="text-sm text-gray-500">Selecione o método e o valor</p>
                        </div>
                        ...
                        <div className="grid grid-cols-4 gap-3 mb-6">
                            {[
                                ...
                                <button
                                    ...
                                    className={`flex flex-col items-center justify-center p-3 rounded-xl border-2 transition-all ${currentMethod === m.id
                                        ? 'border-primary bg-primary/5 text-primary'
                                        : 'border-gray-100 dark:border-zinc-800 hover:border-gray-300 dark:hover:border-zinc-700 bg-white dark:bg-zinc-950'
                                        }`}
                                >
                                    <m.icon className={`w-5 h-5 mb-1 ${currentMethod === m.id ? 'text-primary' : 'text-gray-400'}`} />
                        ...
                        <div className="space-y-4 flex-1">
                            <div>
                                <label className="block text-xs font-bold text-gray-500 uppercase mb-2">Valor a Pagar</label>
                            ...
                            {currentMethod === 'cash' && (
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 uppercase mb-2">Valor Recebido (Troco)</label>
                                    <div className="relative">
                                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 font-bold">R$</span>
                                        <Input
                                            type="number"
                                    ...
                                    {currentChange > 0 && (
                                        <p className="text-right text-green-600 font-bold mt-1">Troco: {formatCurrency(currentChange)}</p>
                                    )}
                                </div>
                            )}
                            ...
                            {currentMethod === 'credit_card' && (
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 uppercase mb-2">Parcelas</label>
                                    <select
                                        value={installments}
                                    ...
                                    >
                                        {[1, 2, 3, 4, 5, 6, 10, 12].map(i => (
                                            <option key={i} value={i}>
                                                {i}x de {formatCurrency((parseFloat(amountToPay) || 0) / i)} sem juros
                                            </option>
                                        ))}
                                    </select>
                                </div>
                    ...
                    <div className="w-full md:w-96 bg-gray-50 dark:bg-zinc-950 p-6 flex flex-col border-l border-gray-100 dark:border-zinc-800">
                        <div className="flex justify-between items-center mb-6">
                            <h4 className="font-bold text-gray-900 dark:text-white">Resumo</h4>
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar mb-4">
                            <h5 className="text-xs font-bold text-gray-500 uppercase mb-3">Pagamentos Adicionados</h5>
                            {payments.length === 0 ? (
                                <p className="text-sm text-gray-400 italic text-center py-4">Nenhum pagamento adicionado</p>
                            ) : (
                                <div className="space-y-2">
                                    {payments.map(p => (
                                        <div key={p.id} className="bg-white dark:bg-zinc-900 p-3 rounded-lg border border-gray-200 dark:border-zinc-800 flex justify-between items-center shadow-sm">
                                            <div>
                                                <p className="font-bold text-sm flex items-center gap-2">
                                                    {p.method === 'cash' && <Banknote className="w-3 h-3" />}
                                                    {p.method === 'credit_card' && <CreditCard className="w-3 h-3" />}
                                                    {p.method === 'debit_card' && <CreditCard className="w-3 h-3" />}
                                                    {p.method === 'pix' && <QrCode className="w-3 h-3" />}
                                                    <span className="capitalize">{p.method.replace('_', ' ')}</span>
                                                </p>
                                                {p.installments && p.installments > 1 && (
                                                    <p className="text-xs text-gray-500">{p.installments}x parcelas</p>
                                                )}
                                            </div>
                                            <div className="flex items-center gap-3">
                                                <span className="font-bold">{formatCurrency(p.amount)}</span>
                                                <button onClick={() => handleRemovePayment(p.id)} className="text-red-400 hover:text-red-600">
                        ...
                        <Button
                            ...
                            disabled={remaining > 0.01 && !(payments.length === 0 && currentMethod === 'cash' && receivedAmount && parseFloat(receivedAmount) >= total)}
                        >
