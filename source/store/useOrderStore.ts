...
export const useOrderStore = create<OrderState>((set) => ({
  ...
  addOrder: (order) => {
    ...
    if (
      !cashRegister.isOpen &&
      (order.paymentMethod === "cash" || order.paymentMethod === "debit_card")
    ) {
      throw new Error(
        "Caixa fechado. Abra o caixa para registrar vendas em dinheiro ou débito.",
      );
    }
    ...
    order.items.forEach((orderItem) => {
      useCatalogStore
        .getState()
        .updateItemStock(orderItem.id, -orderItem.quantity);
    });
    ...
    if (
      ...
      useCashRegisterStore
        .getState()
        .registerSale(order.total, `Venda Pedido #${order.id}`, currentUser);
    }
  ...
  addOrderMessage: (orderId: string, message: Message) =>
    set((state) => {
      const newOrders = state.orders.map((o) =>
        o.id === orderId
  ...
  returnOrderItems: (orderId, itemsToReturn) =>
    set((state) => {
      ...
      itemsToReturn.forEach((returnItem) => {
        ...
        if (!originalItem)
          throw new Error(
            `Item ${returnItem.itemId} não encontrado no pedido.`,
          );
        if (returnItem.quantity > originalItem.quantity)
          throw new Error(
            `Quantidade a devolver maior que a comprada para o item ${originalItem.name}.`,
          );
      ...
      itemsToReturn.forEach((returnItem) => {
        useCatalogStore
          .getState()
          .updateItemStock(returnItem.itemId, returnItem.quantity);
      });
      ...
      if (
        ...
        if (
          ...
          cashRegister.currentBalance < refundTotal
        ) {
          throw new Error(
            "Saldo em caixa insuficiente para realizar o reembolso em dinheiro.",
          );
        }
        useCashRegisterStore
          .getState()
      ...
      const totalReturnedCount = itemsToReturn.reduce(
        (acc, item) => acc + item.quantity,
        0,
      );

      const newStatus: OrderStatus =
        totalReturnedCount >= totalItemsCount
          ? "returned"
          : "partially_returned";
