...
export const useCashRegisterStore = create<CashRegisterStore>((set) => ({
  cashRegister: loadData<CashRegisterState>(
    "businessCashRegister",
    DEFAULT_CASH_REGISTER,
  ),

  openRegister: (startAmount, user) =>
    set(() => {
      const newState = {
        isOpen: true,
        openedAt: new Date(),
  ...
  addCashTransaction: (type, amount, description, user) =>
    set((state) => {
      const newBalance =
        type === "supply"
  ...
  registerSale: (amount, description, user) =>
    set((state) => {
      const newState = {
        ...state.cashRegister,
