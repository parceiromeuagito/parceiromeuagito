...
import { ServiceConfig, PlanTier, UserProfile, IntegrationConfig, BusinessType } from '../types';
...
const DEFAULT_INTEGRATIONS: IntegrationConfig[] = [
  ...
  { id: '3', provider: 'google_reserve', name: 'Google Reservas', connected: false },
...
const loadInitialConfig = (): ServiceConfig => {
  try {
    ...
    if (!parsed.primaryType) {
      parsed.primaryType = parsed.businessTypes && parsed.businessTypes.length > 0
...
export const useBusinessStore = create<BusinessState>((set) => ({
  ...
  updateConfig: (updates: Partial<ServiceConfig>) => set((state) => {
    const newConfig = { ...state.config, ...updates };
    saveData('businessConfig', newConfig);
    return { config: newConfig };
  }),

  toggleIntegration: (providerId: string, apiKey: string) => set((state) => {
  ...
  updatePlan: (newPlan: PlanTier) => set((state) => {
  ...
  toggleBusinessType: (type: BusinessType) => set((state) => {
  ...
  setPrimaryType: (type: BusinessType) => set((state) => {
    ...
    const newConfig = { ...state.config, primaryType: type, businessTypes: newTypes };
    saveData('businessConfig', newConfig);
    return { config: newConfig };
  }),
...
export const getBusinessContext = (types: BusinessType[], config: ServiceConfig) => {
  ...
  const type = config.primaryType || (types && types.length > 0 ? types[0] : 'delivery');
