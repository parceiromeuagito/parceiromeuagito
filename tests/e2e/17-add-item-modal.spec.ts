...
test.describe("AddItemModal - Adicionar Novo Item ao Catálogo", () => {
  ...
  test("AM03 - Deve ter seletor de tipo de negócio (se múltiplos tipos)", async ({
    page,
  }) => {
    await page.waitForTimeout(500);
  ...
  test("AM04 - Deve mudar campos ao selecionar tipo diferente", async ({
    page,
  }) => {
    await page.waitForTimeout(500);
  ...
  test("AM05 - Deve ter área de upload de imagem", async ({ page }) => {
    ...
    const uploadArea = page
      .locator('[class*="upload"], input[type="file"], [role="button"]')
      .filter({
  ...
  test("AM06 - Deve fazer upload de imagem (simulado)", async ({ page }) => {
    ...
    if ((await fileInput.count()) > 0) {
      ...
      const preview = page.locator(
        'img[src*="blob:"], img[src*="data:"], [class*="preview"]',
      );
  ...
  test("AM07 - Deve validar campos obrigatórios", async ({ page }) => {
    ...
    const saveButton = page
      .locator("button")
      .filter({ hasText: /salvar|adicionar|confirmar/i })
      .first();
    ...
    if ((await saveButton.count()) > 0) {
      ...
      const errorMessage = page.locator(
        "text=/obrigatório|preencha|required|campo.*vazio/i",
      );
  ...
  test("AM08 - Deve salvar item com dados válidos", async ({ page }) => {
    ...
    const categoryInput = page
      .locator('input[placeholder*="categoria"], input[name*="category"]')
      .first();
    ...
    if ((await saveButton.count()) > 0) {
      ...
      // Toast de sucesso
      await expect(
        page.locator("text=/sucesso|adicionado|criado/i"),
  ...
  test('AM09 - Deve mostrar campo "Capacidade" para tipo Hotel', async ({
    page,
  }) => {
    await page.waitForTimeout(500);
    ...
    if ((await typeSelect.count()) > 0) {
      ...
      try {
        ...
        const capacityInput = page.locator(
          'input[placeholder*="capacidade"], input[name*="capacity"]',
        );
  ...
  test('AM10 - Deve mostrar campo "Duração" para tipo Scheduling', async ({
    page,
  }) => {
    await page.waitForTimeout(500);
    ...
    if ((await typeSelect.count()) > 0) {
      try {
        ...
        const durationInput = page.locator(
          'input[placeholder*="duração"], input[name*="duration"]',
        );
...
test.describe("AddItemModal - Editar Item Existente", () => {
  ...
  test("EDIT01 - Deve abrir modal em modo edição ao clicar em editar", async ({
    page,
  }) => {
    ...
    if ((await firstItem.count()) > 0) {
      ...
      if ((await editButton.count()) > 0) {
        ...
        const nameInput = page
          .locator('input[name*="name"], input[placeholder*="nome"]')
          .first();
  ...
  test("EDIT02 - Deve atualizar item ao salvar alterações", async ({
    page,
  }) => {
      ...
      if ((await editButton.count()) > 0) {
        ...
        if ((await nameInput.count()) > 0) {
          ...
          if ((await saveButton.count()) > 0) {
            ...
            // Toast de sucesso
            await expect(
              page.locator("text=/atualizado|salvo|sucesso/i"),
            ).toBeVisible({ timeout: 3000 });
  ...
  test("EDIT03 - Deve cancelar edição sem salvar", async ({ page }) => {
    ...
    if ((await firstItem.count()) > 0) {
      // Pegar nome original
      const originalName = await firstItem
        .locator("text=/[A-Za-z]/")
        .first()
        .textContent();
      ...
      if ((await editButton.count()) > 0) {
        ...
        if ((await nameInput.count()) > 0) {
          ...
          const cancelButton = page
            .locator("button")
            .filter({ hasText: /cancelar|fechar/i })
            .first();
...
test.describe("AddItemModal - Campos Condicionais por Tipo", () => {
  ...
  test("COND01 - Hotel: Campo de capacidade máxima", async ({ page }) => {
    ...
    if ((await typeSelect.count()) > 0) {
      ...
      const hotelOption = options.find((opt) =>
      ...
      if (hotelOption) {
        ...
        const capacityInput = page
          .locator('input[name*="capacity"], input[placeholder*="capacidade"]')
          .first();
  ...
  test("COND02 - Scheduling: Campo de duração em minutos", async ({ page }) => {
    ...
    if ((await typeSelect.count()) > 0) {
      ...
      if (
        options.some(
          (opt) =>
            opt.toLowerCase().includes("agend") ||
            opt.toLowerCase().includes("schedul"),
        )
      ) {
        await typeSelect.selectOption({ index: 1 });
        await page.waitForTimeout(500);
        ...
        const durationInput = page
          .locator('input[name*="duration"], input[placeholder*="duração"]')
          .first();
  ...
  test("COND03 - Tickets/Ecommerce: Campo de estoque inicial", async ({
    page,
  }) => {
    const typeSelect = page.locator("select").first();
    ...
    if ((await typeSelect.count()) > 0) {
      ...
      const stockInput = page
        .locator('input[name*="stock"], input[placeholder*="estoque"]')
        .first();
