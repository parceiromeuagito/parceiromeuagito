...
test.describe("Login - Autenticação e Seleção de Perfil", () => {
  ...
  test("L01 - Deve fazer login e ir para seleção de perfil", async ({
    page,
  }) => {
  ...
  test('L02 - Deve bloquear login sem "parceiro" ou "partner" no email', async ({
    page,
  }) => {
    ...
    await expect(
      page.locator("text=negado").or(page.locator("text=parceiro")),
  ...
  test("SP02 - Deve abrir teclado numérico ao selecionar perfil", async ({
    page,
  }) => {
    await loginToSelectProfile(page);
  ...
  test("SP04 - Deve mostrar erro com PIN incorreto", async ({ page }) => {
    ...
    // Deve mostrar erro
    await expect(
      page.locator("text=incorreto").or(page.locator("text=Tente")),
    ).toBeVisible({ timeout: 5000 });
  });

  test("SP05 - Deve acessar dashboard com PIN correto como Admin", async ({
    page,
  }) => {
    ...
    await expect(
      page.locator("text=Dashboard").or(page.locator("text=Visão Geral")),
    ).toBeVisible();
  });
  ...
  test("PR01 - Deve redirecionar para /login ao acessar /dashboard sem autenticação", async ({
    page,
  }) => {
  ...
  test("PR02 - Deve redirecionar para /login ao acessar /select-profile sem autenticação", async ({
    page,
  }) => {
    await clearSession(page);
