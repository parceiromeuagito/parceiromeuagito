...
test.describe('Orders - Gestão de Pedidos', () => {
    ...
    test('O02 - Deve aceitar pedido pendente', async ({ page }) => {
        ...
        if (await acceptButton.count() > 0) {
            ...
            await expect(page.locator('text=/aceita|aceito/i')).toBeVisible({ timeout: 3000 });
        }
    });

    test('O03 - Deve recusar pedido pendente', async ({ page }) => {
        const rejectButton = page.locator('button').filter({ hasText: /Recusar/i }).first();

        if (await rejectButton.count() > 0) {
            await rejectButton.click();

            // Verificar toast
            await expect(page.locator('text=/recusad|rejeitad/i')).toBeVisible({ timeout: 3000 });
        }
    });
    ...
    test('O05 - Deve avançar pedido (preparing → ready)', async ({ page }) => {
        ...
        const advanceButtons = page.locator('button').filter({ hasText: /Avançar|Marcar.*Pronto/i });
    ...
    test('O06 - Deve avançar pedido (ready → delivering)', async ({ page }) => {
        const advanceButtons = page.locator('button').filter({ hasText: /Avançar|Enviar.*Entrega/i });
    ...
    test('O07 - Deve concluir pedido (delivering → completed)', async ({ page }) => {
        const completeButton = page.locator('button').filter({ hasText: /Concluir|Finalizar/i }).first();

        if (await completeButton.count() > 0) {
            await completeButton.click();

            // Verificar toast
            await expect(page.locator('text=/concluído|finalizado/i')).toBeVisible({ timeout: 3000 });
        }
    });

    test('O08 - Deve mostrar contador de pedidos por coluna', async ({ page }) => {
        // Procurar badges com números
        const badges = page.locator('[class*="badge"], [class*="count"]').filter({ hasText: /\d+/ });
    ...
    test('O09 - Deve ativar toggle de auto-aceite', async ({ page }) => {
        const autoAcceptToggle = page.locator('input[type="checkbox"]').filter({ hasText: /auto/i }).or(
            page.locator('label').filter({ hasText: /auto.*aceite/i }).locator('input')
        );
    ...
    test('O11 - Deve abrir modal de detalhes ao clicar no card', async ({ page }) => {
    ...
    test('O12 - Deve ter animação de transição ao mover pedido', async ({ page }) => {
        // Verificar se há classes de animação do Framer Motion
        const animatedElements = page.locator('[style*="transform"], [class*="animate"]');
    ...
    test('O13 - Deve ter botão de chat (Pro)', async ({ page }) => {
        const chatButton = page.locator('button, [class*="chat"]').filter({ hasText: /chat|mensagem/i });
    ...
    test('O14 - Deve bloquear chat se Starter', async ({ page }) => {
        ...
        if (isStarter) {
            ...
            if (await chatButton.count() > 0) {
                ...
                await expect(page.locator('text=/upgrade|Pro/i')).toBeVisible({ timeout: 3000 });
            }
        }
    });
    ...
    test('O15 - Deve mostrar badge de mensagens não lidas', async ({ page }) => {
        ...
        const messageBadges = page.locator('[class*="badge"]').filter({ hasText: /\d+/ });
    ...
    test('O16 - Deve alternar para visualização em lista', async ({ page }) => {
        ...
        const listButton = page.locator('button').filter({ hasText: /lista/i }).or(
            page.locator('button[aria-label*="list"], [data-view="list"]')
        );
    ...
    test('O17 - Deve mostrar badges de status coloridos na lista', async ({ page }) => {
        // Alternar para lista primeiro
        const listButton = page.locator('button').filter({ hasText: /lista/i }).first();
    ...
    test('O19 - Deve ter botão Avançar na lista', async ({ page }) => {
        ...
        const advanceButtons = page.locator('button').filter({ hasText: /Avançar/i });
    ...
    test('O23 - Deve mostrar empty state se não houver pedidos ativos', async ({ page }) => {
    ...
    test('O25 - Deve abrir modal ao clicar no card da lista', async ({ page }) => {
        const listButton = page.locator('button').filter({ hasText: /lista/i }).first();
        if (await listButton.count() > 0) await listButton.click();
    ...
    test('O26 - Deve alternar para visualização calendário', async ({ page }) => {
        const calendarButton = page.locator('button').filter({ hasText: /calendário|agenda/i }).or(
            page.locator('[data-view="calendar"]')
        );
    ...
    test('O27 - Deve agrupar pedidos por data', async ({ page }) => {
        const calendarButton = page.locator('button').filter({ hasText: /calendário/i }).first();
        if (await calendarButton.count() > 0) {
            await calendarButton.click();
            await page.waitForTimeout(500);

            // Procurar agrupamentos de data
            const dateHeaders = page.locator('text=/\\d{2}\\/\\d{2}|Segunda|Terça|Quarta/i');
    ...
    test('O29 - Deve exibir detalhes específicos de hotel (check-in, noites)', async ({ page }) => {
        const calendarButton = page.locator('button').filter({ hasText: /calendário/i }).first();
    ...
    test('O30 - Deve exibir detalhes específicos de reserva (data, pessoas)', async ({ page }) => {
        const calendarButton = page.locator('button').filter({ hasText: /calendário/i }).first();
        if (await calendarButton.count() > 0) await calendarButton.click();
    ...
    test('O31 - Deve exibir detalhes específicos de tickets (assento)', async ({ page }) => {
        const calendarButton = page.locator('button').filter({ hasText: /calendário/i }).first();
        if (await calendarButton.count() > 0) await calendarButton.click();
    ...
    test('O32 - Deve mostrar empty state se não houver agendamentos', async ({ page }) => {
        const calendarButton = page.locator('button').filter({ hasText: /calendário/i }).first();
        if (await calendarButton.count() > 0) await calendarButton.click();
        ...
        const emptyState = page.locator('text=/nenhum agendamento|sem agendamentos/i');
        const count = await emptyState.count();
        expect(count).toBeGreaterThanOrEqual(0);
    });

    test('O33 - Deve abrir modal ao clicar no card do calendário', async ({ page }) => {
        const calendarButton = page.locator('button').filter({ hasText: /calendário/i }).first();
        if (await calendarButton.count() > 0) await calendarButton.click();
    ...
    test('O34 - Deve ter aba "Histórico"', async ({ page }) => {
        const historyTab = page.locator('button, [role="tab"]').filter({ hasText: /histórico|history/i });
        ...
        if (await historyTab.count() > 0) {
            ...
            await expect(page.locator('text=/concluído|cancelado|finalizado/i')).toBeVisible({ timeout: 5000 });
        }
    });

    test('O35 - Deve exibir badge "Concluído" em verde', async ({ page }) => {
        const historyTab = page.locator('button').filter({ hasText: /histórico/i }).first();
        if (await historyTab.count() > 0) await historyTab.click();

        await page.waitForTimeout(500);

        // Procurar badge verde de concluído
        const completeBadge = page.locator('[class*="green"], [class*="success"]').filter({ hasText: /concluído/i });
