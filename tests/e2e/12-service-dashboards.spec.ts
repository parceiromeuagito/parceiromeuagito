...
test.describe("Service Dashboards - Dashboards Adaptativos", () => {
  ...
  test("SD01 - Restaurant: Deve exibir métricas de cozinha", async ({
    page,
  }) => {
    await page.waitForTimeout(1000);

    // Procurar métricas relacionadas a restaurante
    const kitchenMetrics = page.locator(
      "text=/tempo.*médio|pedidos.*cozinha|mesas/i",
    );
  ...
  test("SD02 - Restaurant: Deve exibir status de mesas", async ({ page }) => {
    ...
    const tableStatus = page.locator(
      "text=/mesa.*\d+|mesas.*ocupadas|mesas.*livres/i",
    );
    const count = await tableStatus.count();
    expect(count).toBeGreaterThanOrEqual(0);
  });

  test("SD03 - Restaurant: Deve exibir pedidos por categoria (bebida/comida)", async ({
    page,
  }) => {
    await page.waitForTimeout(1000);
    ...
    const categoryBreakdown = page.locator(
      "text=/bebidas|comidas|entradas|sobremesas/i",
    );
    const count = await categoryBreakdown.count();
    expect(count).toBeGreaterThanOrEqual(0);
  });

  test("SD04 - Restaurant: Deve ter atalho rápido para cozinha", async ({
    page,
  }) => {
    await page.waitForTimeout(1000);
    ...
    const kitchenLink = page
      .locator("button, a")
      .filter({ hasText: /cozinha|kitchen/i })
      .first();
  ...
  test("SD05 - Hotel: Deve exibir status de quartos", async ({ page }) => {
    ...
    const roomStatus = page.locator(
      "text=/quartos.*disponíveis|ocupados|manutenção/i",
    );
  ...
  test("SD09 - Scheduling: Deve exibir agenda do dia", async ({ page }) => {
    ...
    const schedule = page.locator(
      "text=/agenda.*hoje|agendamentos.*hoje|horários/i",
    );
    const count = await schedule.count();
    expect(count).toBeGreaterThanOrEqual(0);
  });

  test("SD10 - Scheduling: Deve exibir próximos agendamentos", async ({
    page,
  }) => {
    await page.waitForTimeout(1000);

    // Procurar lista de próximos agendamentos
    const upcomingAppointments = page.locator(
      "text=/próxim.*agendamento|próxim.*hora/i",
    );
    const count = await upcomingAppointments.count();
    expect(count).toBeGreaterThanOrEqual(0);
  });

  test("SD11 - Scheduling: Deve exibir taxa de ocupação por horário", async ({
    page,
  }) => {
    await page.waitForTimeout(1000);
  ...
  test("SD12 - Scheduling: Deve ter atalho para criar agendamento", async ({
    page,
  }) => {
    await page.waitForTimeout(1000);
    ...
    const newAppointmentButton = page
      .locator("button")
      .filter({ hasText: /novo.*agendamento|agendar/i })
      .first();
  ...
  test("SD13 - Tickets: Deve exibir eventos futuros", async ({ page }) => {
    ...
    const upcomingEvents = page.locator(
      "text=/próxim.*evento|eventos.*futur/i",
    );
    const count = await upcomingEvents.count();
    expect(count).toBeGreaterThanOrEqual(0);
  });

  test("SD14 - Tickets: Deve exibir ingressos vendidos hoje", async ({
    page,
  }) => {
    await page.waitForTimeout(1000);

    // Procurar métrica de ingressos
    const ticketsSold = page.locator(
      "text=/ingressos.*vendidos|tickets.*sold/i",
    );
    const count = await ticketsSold.count();
    expect(count).toBeGreaterThanOrEqual(0);
  });

  test("SD15 - Tickets: Deve exibir taxa de ocupação por evento", async ({
    page,
  }) => {
    await page.waitForTimeout(1000);
  ...
  test("SD16 - Tickets: Deve ter atalho para criar evento", async ({
    page,
  }) => {
    await page.waitForTimeout(1000);
    ...
    const newEventButton = page
      .locator("button")
      .filter({ hasText: /novo.*evento|criar.*evento/i })
      .first();
  ...
  test("SD17 - Delivery: Deve exibir entregas em andamento", async ({
    page,
  }) => {
    await page.waitForTimeout(1000);

    // Procurar métricas de entrega
    const activeDeliveries = page.locator(
      "text=/entregas.*andamento|em.*rota/i",
    );
    const count = await activeDeliveries.count();
    expect(count).toBeGreaterThanOrEqual(0);
  });

  test("SD18 - Delivery: Deve exibir tempo médio de entrega", async ({
    page,
  }) => {
    await page.waitForTimeout(1000);
    ...
    const avgTime = page.locator(
      "text=/tempo.*médio|média.*entrega|\d+.*min/i",
    );
  ...
  test("SD20 - Delivery: Deve ter mapa de entregas (se disponível)", async ({
    page,
  }) => {
    await page.waitForTimeout(1000);
...
test.describe("Service Dashboard Switching - Alternância de Tipos", () => {
  ...
  test("SWITCH01 - Deve permitir alternar tipo de negócio em Settings", async ({
    page,
  }) => {
    ...
    const businessTypeCards = page
      .locator('[class*="card"], button')
      .filter({ hasText: /delivery|hotel|restaurant/i });
  ...
  test("SWITCH03 - Deve habilitar/desabilitar tipos secundários (extensões)", async ({
    page,
  }) => {
    await page.goto("/dashboard/settings");
    await page.waitForTimeout(1000);
  ...
  test("SWITCH04 - Deve validar limite de tipos por plano", async ({
    page,
  }) => {
    await page.goto("/dashboard/settings");
    await page.waitForTimeout(1000);
    ...
    const businessCards = await page
      .locator('[class*="card"], button')
      .filter({ hasText: /delivery|hotel|restaurant|ticket/i })
      .all();
