...
test.describe("Category Management - Gestão de Categorias", () => {
  ...
  test("CAT03 - Deve adicionar nova categoria", async ({ page }) => {
    ...
    if ((await manageBtn.count()) > 0) {
      ...
      const addBtn = page
        .locator("button")
        .filter({ hasText: /adicionar|nova/i })
        .first();
      if ((await addBtn.count()) > 0) {
        ...
        if ((await nameInput.count()) > 0) {
          ...
          if ((await saveBtn.count()) > 0) {
            ...
            await expect(page.locator("text=/sucesso|adicionad/i")).toBeVisible(
              { timeout: 3000 },
            );
          }
        }
  ...
  test("CAT04 - Deve editar nome da categoria", async ({ page }) => {
    const categoryBtn = page
      .locator("button")
      .filter({ hasText: /bebida|comida/i })
      .first();
  ...
  test("CAT05 - Deve ordenar categorias por drag and drop", async ({
    page,
  }) => {
  ...
  test("CAT06 - Deve ter ícone associado a cada categoria", async ({
    page,
  }) => {
    const categoryIcons = page.locator(
      '[class*="category"] svg, [class*="category"] [class*="icon"]',
    );
...
test.describe("Security Gate - PIN de Segurança", () => {
  ...
  test("SG03 - Deve mascarar PIN digitado", async ({ page }) => {
    ...
    if ((await resetBtn.count()) > 0) {
      ...
      const display = page
        .locator('[class*="pin"], [class*="display"]')
        .first();
  ...
  test("SG06 - Deve ter botão backspace", async ({ page }) => {
    ...
    if ((await resetBtn.count()) > 0) {
      ...
      const backspaceBtn = page
        .locator('button[aria-label*="backspace"], button')
        .filter({
...
test.describe("Business Context - Adaptação por Tipo", () => {
  ...
  test("BC01 - Dashboard deve adaptar widgets por tipo de negócio", async ({
    page,
  }) => {
    ...
    const widgets = page.locator(
      '[class*="widget"], [class*="service"], [data-service]',
    );
  ...
  test("BC10 - Deve persistir tipo de negócio após reload", async ({
    page,
  }) => {
    await page.goto("/dashboard");
    await page.waitForTimeout(1000);
