...
test.describe('Menu - Catálogo de Itens', () => {
    ...
    test('M02 - Deve filtrar por categoria', async ({ page }) => {
        ...
        const categoryButtons = page.locator('button').filter({ hasText: /categoria|todos|bebida|comida/i });
    ...
    test('M04 - Deve ter botão "Novo Item"', async ({ page }) => {
        ...
        const addButton = page.locator('button').filter({ hasText: /novo.*item|adicionar|criar/i }).first();
    ...
    test('M06 - Deve ter botão de editar item', async ({ page }) => {
        ...
        if (await itemCard.count() > 0) {
            ...
            const editButton = page.locator('button[aria-label*="edit"], button').filter({ has: page.locator('svg') }).first();
    ...
    test('M07 - Deve ter botão de repor estoque', async ({ page }) => {
        ...
        if (await itemCard.count() > 0) {
            ...
            const stockButton = page.locator('button').filter({ hasText: /repor|estoque/i });
    ...
    test('M08 - Deve ter botão de deletar item', async ({ page }) => {
        ...
        if (await itemCard.count() > 0) {
            ...
            const deleteButton = page.locator('button').filter({ has: page.locator('svg[class*="trash"]') });
            const count = await deleteButton.count();
            expect(count).toBeGreaterThanOrEqual(0);
        }
    });

    test('M09 - Deve exibir badge "Ativo" em item disponível', async ({ page }) => {
        await page.waitForTimeout(1000);

        // Procurar badges de status
        const activeBadge = page.locator('[class*="badge"], [class*="status"]').filter({ hasText: /ativo|disponível/i });
        const count = await activeBadge.count();
        expect(count).toBeGreaterThanOrEqual(0);
    });

    test('M10 - Deve exibir badge "Inativo" em item indisponível', async ({ page }) => {
        await page.waitForTimeout(1000);

        const inactiveBadge = page.locator('[class*="badge"]').filter({ hasText: /inativo|indisponível/i });
    ...
    test('M12 - Deve exibir badge de categoria', async ({ page }) => {
        ...
        const categoryBadges = page.locator('[class*="badge"], [class*="category"]');
...
test.describe('Customers - Gestão de Clientes', () => {
    ...
    test('C01 - Deve exibir tabela de clientes', async ({ page }) => {
        ...
        const customersTable = page.locator('table, [class*="customer"], [class*="list"]');
    ...
    test('C02 - Deve ter campo de busca por nome/telefone', async ({ page }) => {
        ...
        const searchInput = page.locator('input[type="search"], input[placeholder*="busca"], input[placeholder*="cliente"]').first();
    ...
    test('C03 - Deve exibir badge VIP para clientes com gasto > R$ 1000', async ({ page }) => {
        await page.waitForTimeout(1000);

        // Procurar badges VIP
        const vipBadges = page.locator('[class*="badge"]').filter({ hasText: /vip/i });
        const count = await vipBadges.count();
        expect(count).toBeGreaterThanOrEqual(0);
    });

    test('C04 - Deve exibir badge verde para status "Active"', async ({ page }) => {
        await page.waitForTimeout(1000);

        // Procurar badges de status ativo
        const activeBadges = page.locator('[class*="green"], [class*="active"]').filter({ hasText: /ativo|active/i });
        const count = await activeBadges.count();
        expect(count).toBeGreaterThanOrEqual(0);
    });

    test('C05 - Deve exibir badge cinza para status "Inactive"', async ({ page }) => {
        await page.waitForTimeout(1000);

        const inactiveBadges = page.locator('[class*="gray"], [class*="inactive"]').filter({ hasText: /inativo|inactive/i });
    ...
    test('C06 - Deve exibir total de pedidos por cliente', async ({ page }) => {
        ...
        const orderCounts = page.locator('td, [class*="orders"]').filter({ hasText: /\d+/ });
    ...
    test('C08 - Deve exibir empty state se não houver clientes', async ({ page }) => {
        await page.waitForTimeout(1000);

        // Procurar mensagem de lista vazia
        const emptyState = page.locator('text=/nenhum cliente|sem clientes|vazio/i');
