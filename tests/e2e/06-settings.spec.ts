...
test.describe('Settings - Configurações', () => {
    ...
    test('S01 - Deve exibir aba "Geral" ativa por padrão', async ({ page }) => {
        ...
        // Verificar aba ativa
        const generalTab = page.locator('[role="tab"], button').filter({ hasText: /geral|general/i }).first();
    ...
    test('S02 - Deve ter cards de seleção de tipo de negócio principal', async ({ page }) => {
        await page.waitForTimeout(1000);

        // Procurar cards de tipo de negócio
        const businessTypeCards = page.locator('[class*="card"], [data-business-type]');
    ...
    test('S03 - Deve selecionar tipo de negócio principal', async ({ page }) => {
        ...
        const businessCard = page.locator('[class*="card"], button').filter({ hasText: /delivery|reserva|hotel/i }).first();
    ...
    test('S04 - Deve ter toggle para tipos de negócio secundários (extensões)', async ({ page }) => {
        await page.waitForTimeout(1000);
    ...
    test('S05 - Deve ter campos de detalhes do estabelecimento', async ({ page }) => {
        await page.waitForTimeout(1000);

        // Procurar inputs de nome fantasia e CNPJ
        const nameInput = page.locator('input[placeholder*="fantasia"], input[name*="name"]').first();
    ...
    test('S05C - Deve salvar configurações ao clicar "Salvar"', async ({ page }) => {
        await page.waitForTimeout(1000);
        ...
        if (await saveButton.count() > 0) {
            ...
            // Toast de sucesso
            await expect(page.locator('text=/salvo|configurações.*salvas/i')).toBeVisible({ timeout: 3000 });
        }
    });

    test('S05D - Deve ter zona de perigo com botão "Resetar Sistema"', async ({ page }) => {
        await page.waitForTimeout(1000);
        ...
        const resetButton = page.locator('button').filter({ hasText: /resetar.*sistema|reset/i }).first();
    ...
    test('S06 - Deve navegar para aba "Impressora"', async ({ page }) => {
        ...
        const printerTab = page.locator('[role="tab"], button').filter({ hasText: /impressora|printer/i }).first();
    ...
    test('S07 - Deve ter botões de seleção de papel (58mm/80mm)', async ({ page }) => {
        await page.waitForTimeout(1000);

        const printerTab = page.locator('button').filter({ hasText: /impressora/i }).first();
        if (await printerTab.count() > 0) await printerTab.click();
        await page.waitForTimeout(500);

        // Procurar botões de tamanho
        const paperButtons = page.locator('button').filter({ hasText: /58mm|80mm/i });
        const count = await paperButtons.count();
        expect(count).toBeGreaterThanOrEqual(2);
    });

    test('S08 - Deve ter campos de cabeçalho e rodapé customizados', async ({ page }) => {
        await page.waitForTimeout(1000);
        ...
        const headerInput = page.locator('input[placeholder*="cabeçalho"], textarea[placeholder*="cabeçalho"]').first();
        const footerInput = page.locator('input[placeholder*="rodapé"], textarea[placeholder*="rodapé"]').first();

        const count = await headerInput.count() + await footerInput.count();
        expect(count).toBeGreaterThanOrEqual(1);
    });

    test('S09 - Deve ter toggle "Auto-imprimir ao aceitar pedido"', async ({ page }) => {
        await page.waitForTimeout(1000);
        ...
        const autoPrintToggle = page.locator('input[type="checkbox"]').filter({ hasText: /auto/i }).or(
            page.locator('label').filter({ hasText: /auto.*imprimir/i })
        );
    ...
    test('S10 - Deve ter botão "Imprimir Teste"', async ({ page }) => {
        ...
        const testButton = page.locator('button').filter({ hasText: /imprimir.*teste|teste.*impressão/i }).first();
    ...
    test('S11 - Deve navegar para aba "Planos"', async ({ page }) => {
        ...
        const plansTab = page.locator('[role="tab"], button').filter({ hasText: /planos|plans/i }).first();
    ...
    test('S12 - Deve exibir 3 cards de planos (Starter/Pro/Enterprise)', async ({ page }) => {
        await page.waitForTimeout(1000);

        const plansTab = page.locator('button').filter({ hasText: /planos/i }).first();
        ...
        const count = await starterCard.count() + await proCard.count() + await enterpriseCard.count();
        expect(count).toBeGreaterThanOrEqual(3);
    });

    test('S12B - Deve ter botão "Escolher Plano" em cada card', async ({ page }) => {
        await page.waitForTimeout(1000);
        ...
        const choosePlanButtons = page.locator('button').filter({ hasText: /escolher|selecionar|upgrade/i });
    ...
    test('S13 - Deve navegar para aba "Integrações" (se Pro/Enterprise)', async ({ page }) => {
        await page.waitForTimeout(1000);

        const integrationsTab = page.locator('[role="tab"], button').filter({ hasText: /integrações|integrations/i }).first();
    ...
    test('S14 - Deve bloquear integrações se plano Starter', async ({ page }) => {
        ...
        if (isStarter) {
            const integrationsTab = page.locator('button').filter({ hasText: /integrações/i }).first();
    ...
    test('S14B - Deve ter botões "Conectar" nas integrações (Pro)', async ({ page }) => {
        await page.waitForTimeout(1000);
        ...
        if (await integrationsTab.count() > 0) {
            ...
            const connectButtons = page.locator('button').filter({ hasText: /conectar|connect/i });
            const count = await connectButtons.count();
            expect(count).toBeGreaterThanOrEqual(0);
        }
    });

    test('S14C - Deve ter campos de API Key nas integrações', async ({ page }) => {
        await page.waitForTimeout(1000);
        ...
        if (await integrationsTab.count() > 0) {
            ...
            const apiKeyInputs = page.locator('input[placeholder*="API"], input[name*="api"]');
    ...
    test('S15 - Deve navegar para aba "Equipe" (se existe)', async ({ page }) => {
        ...
        const teamTab = page.locator('[role="tab"], button').filter({ hasText: /equipe|team|usuários/i }).first();
    ...
    test('S15B - Deve resetar sistema com confirmação', async ({ page }) => {
        ...
        const generalTab = page.locator('button').filter({ hasText: /geral/i }).first();
        ...
        const resetButton = page.locator('button').filter({ hasText: /resetar/i }).first();
    ...
    test('S15C - Deve validar limite de tipos de negócio por plano', async ({ page }) => {
        await page.waitForTimeout(1000);

        // Tentar adicionar muitos tipos de negócio
        const businessCards = await page.locator('[class*="card"], button').filter({ hasText: /delivery|hotel|reserva/i }).all();
    ...
    test('S15D - Deve ter select de número de cópias (1-3) na impressora', async ({ page }) => {
        await page.waitForTimeout(1000);
        ...
        const copiesSelect = page.locator('select, [role="combobox"]').filter({ hasText: /cópias|vias/i });
    ...
    test('S15E - Deve ter select de tamanho de fonte na impressora', async ({ page }) => {
        await page.waitForTimeout(1000);
        ...
        const fontSelect = page.locator('select, [role="combobox"]').filter({ hasText: /fonte|tamanho/i });
