...
test.describe("Stock Management - Controle de Estoque", () => {
  ...
  test("ST01 - Deve exibir quantidade de estoque em cada item do menu", async ({
    page,
  }) => {
    ...
    const stockBadges = page.locator(
      "text=/estoque.*\d+|\d+.*unid|\d+.*disponível/i",
    );
    const count = await stockBadges.count();
    expect(count).toBeGreaterThanOrEqual(0);
  });

  test("ST02 - Deve destacar itens com estoque baixo (< 10 unidades)", async ({
    page,
  }) => {
    await page.goto("/dashboard/menu");
    await page.waitForLoadState("networkidle");
    ...
    const lowStockBadges = page
      .locator('[class*="yellow"], [class*="warning"], [class*="orange"]')
      .filter({ hasText: /estoque.*baixo|baixo/i });
    const count = await lowStockBadges.count();
    expect(count).toBeGreaterThanOrEqual(0);
  });

  test("ST03 - Deve destacar itens sem estoque (vermelho)", async ({
    page,
  }) => {
    await page.goto("/dashboard/menu");
    await page.waitForLoadState("networkidle");
    ...
    const outOfStockBadges = page
      .locator('[class*="red"], [class*="danger"]')
      .filter({ hasText: /sem.*estoque|esgotado|indisponível/i });
    const count = await outOfStockBadges.count();
    expect(count).toBeGreaterThanOrEqual(0);
  });

  test("ST04 - Deve exibir ícone de alerta em itens com estoque baixo", async ({
    page,
  }) => {
    await page.goto("/dashboard/menu");
    await page.waitForLoadState("networkidle");
  ...
  test("ST05 - Deve filtrar apenas itens com estoque baixo", async ({
    page,
  }) => {
    await page.goto("/dashboard/menu");
    await page.waitForLoadState("networkidle");
    ...
    const lowStockFilter = page
      .locator("button")
      .filter({ hasText: /estoque.*baixo/i })
      .first();
  ...
  test("ST06 - Deve abrir modal de reposição ao clicar no botão", async ({
    page,
  }) => {
    await page.goto("/dashboard/menu");
    await page.waitForLoadState("networkidle");
    ...
    if ((await firstItem.count()) > 0) {
      ...
      if ((await restockButton.count()) > 0) {
        ...
        const quantityInput = page
          .locator('input[type="number"], input[placeholder*="quantidade"]')
          .first();
      ...
      if ((await restockButton.count()) > 0) {
        ...
        if ((await quantityInput.count()) > 0) {
          ...
          const confirmButton = page
            .locator("button")
            .filter({ hasText: /confirmar|adicionar/i })
            .first();
          if ((await confirmButton.count()) > 0) {
            ...
            // Toast de sucesso
            await expect(
              page.locator("text=/estoque.*atualizado|sucesso/i"),
        ...
        if ((await quantityInput.count()) > 0) {
          ...
          if ((await confirmButton.count()) > 0) {
            ...
            if (!isDisabled) {
              await confirmButton.click();
              await expect(
                page.locator("text=/valor.*inválido|maior.*zero/i"),
              ).toBeVisible({ timeout: 3000 });
            }
          }
        }
  ...
  test("ST10 - Deve exibir novo estoque após reposição", async ({ page }) => {
    ...
    if ((await firstItem.count()) > 0) {
      // Pegar estoque atual
      const currentStockText = await firstItem
        .locator("text=/estoque|\d+.*unid/i")
        .first()
        .textContent();
      ...
      if ((await restockButton.count()) > 0) {
        ...
        if ((await quantityInput.count()) > 0) {
          ...
          if ((await confirmButton.count()) > 0) {
            ...
            const newStockText = await firstItem
              .locator("text=/estoque|\d+.*unid/i")
              .first()
              .textContent();
  ...
  test("ST11 - Deve verificar estoque antes de aceitar pedido", async ({
    page,
  }) => {
    ...
    if ((await acceptButton.count()) > 0) {
      ...
      const stockAlert = page.locator(
        "text=/sem.*estoque|indisponível|estoque.*insuficiente/i",
      );
  ...
  test("ST12 - Deve bloquear aceitação se item sem estoque", async ({
    page,
  }) => {
  ...
  test("ST14 - Deve mostrar histórico de movimentações de estoque", async ({
    page,
  }) => {
    await page.goto("/dashboard/menu");
    await page.waitForLoadState("networkidle");
    ...
    if ((await firstItem.count()) > 0) {
      ...
      const history = page.locator(
        "text=/histórico|movimentações|entradas.*saídas/i",
      );
  ...
  test("ST15 - Deve exibir notificação de estoque baixo no dashboard", async ({
    page,
  }) => {
    ...
    const stockAlert = page
      .locator('[class*="alert"], [class*="warning"]')
      .filter({ hasText: /estoque.*baixo/i });
    const count = await stockAlert.count();
    expect(count).toBeGreaterThanOrEqual(0);
  });

  test("ST16 - Deve desativar item automaticamente quando estoque = 0", async ({
    page,
  }) => {
    await page.goto("/dashboard/menu");
    await page.waitForLoadState("networkidle");
        ...
        if ((await quantityInput.count()) > 0) {
          ...
          if ((await confirmButton.count()) > 0) {
            ...
            const activeItem = page
              .locator('[class*="active"]:not([class*="inactive"])')
              .first();
  ...
  test("ST18 - Deve enviar notificação ao atingir limite crítico (5 unidades)", async ({
    page,
  }) => {
    await page.goto("/dashboard");
    await page.waitForLoadState("networkidle");
    ...
    const criticalAlert = page
      .locator('[class*="critical"], [class*="danger"]')
      .filter({ hasText: /crítico|urgente|5.*unid/i });
