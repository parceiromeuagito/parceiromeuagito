...
test.describe('AI Services - Previsões e Otimizações', () => {
    ...
    test('AI01 - Dashboard: Deve exibir AI Insights Card (Enterprise)', async ({ page }) => {
        await page.goto('/dashboard');
        await page.waitForTimeout(1500);

        const aiCard = page.locator('[class*="ai"], [class*="insight"]').filter({
    ...
    test('AI03 - Deve exibir nível de confiança da previsão', async ({ page }) => {
        await page.goto('/dashboard');
        await page.waitForTimeout(1500);
    ...
    test('AI05 - Deve ter botão "Criar Ação" que abre Creative Studio', async ({ page }) => {
        await page.goto('/dashboard');
        await page.waitForTimeout(1500);

        const actionBtn = page.locator('button').filter({ hasText: /criar.*ação|action/i }).first();
    ...
    test('AI06 - Creative Studio: Quick actions devem gerar campanhas', async ({ page }) => {
        await page.goto('/dashboard/creative-studio');
        await page.waitForTimeout(1000);

        const rainyBtn = page.locator('button').filter({ hasText: /chuvoso|chuva/i }).first();
        ...
        if (await rainyBtn.count() > 0) {
            ...
            const campaignPreview = page.locator('[class*="preview"], [class*="campaign"]');
    ...
    test('AI07 - Campanha gerada deve ter título criativo', async ({ page }) => {
        ...
        const actionBtn = page.locator('button').filter({ hasText: /chuvoso|vendas/i }).first();
    ...
    test('AI08 - Campanha deve ter copy persuasivo', async ({ page }) => {
        ...
        const actionBtn = page.locator('button').filter({ hasText: /chuvoso/i }).first();
    ...
    test('AI09 - Deve gerar diferentes campanhas por insight type', async ({ page }) => {
        await page.goto('/dashboard/creative-studio');
        await page.waitForTimeout(1000);
        ...
        for (const action of actions) {
            ...
            if (await btn.count() > 0) {
                ...
                const backBtn = page.locator('button').filter({ hasText: /voltar/i }).first();
    ...
    test('AI10 - Deve calcular estimativa de alcance baseada em raio', async ({ page }) => {
        await page.goto('/dashboard/creative-studio');
        await page.waitForTimeout(1000);
        ...
        if (await actionBtn.count() > 0) {
            ...
            const nextBtn = page.locator('button').filter({ hasText: /próximo|público/i }).first();
    ...
    test('AI14 - Deve sugerir desconto baseado em análise', async ({ page }) => {
        ...
        const actionBtn = page.locator('button').filter({ hasText: /vendas.*baixas/i }).first();
    ...
    test('AI15 - Deve bloquear AI features no plano Starter', async ({ page }) => {
        ...
        if (isStarter) {
            ...
            const upgradeMsg = page.locator('text=/upgrade.*enterprise|enterprise.*recurso/i');
