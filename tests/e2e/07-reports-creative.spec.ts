...
test.describe('Reports - Relatórios Gerenciais', () => {
    ...
    test('R02 - Deve exibir aba "Vendas" ativa', async ({ page }) => {
        ...
        if (!isBlocked) {
            const salesTab = page.locator('[role="tab"], button').filter({ hasText: /vendas|sales/i }).first();
    ...
    test('R03 - Deve exibir aba "Caixa Diário"', async ({ page }) => {
        ...
        if (!isBlocked) {
            const cashTab = page.locator('[role="tab"], button').filter({ hasText: /caixa|cash/i }).first();
    ...
    test('R04 - Deve exibir aba "Produtos"', async ({ page }) => {
        ...
        if (!isBlocked) {
            const productsTab = page.locator('[role="tab"], button').filter({ hasText: /produtos|products/i }).first();
    ...
    test('R05 - Deve exibir gráfico de pizza (Métodos de Pagamento)', async ({ page }) => {
        await page.waitForTimeout(1000);
    ...
    test('R06 - Deve exibir gráfico de barras (Evolução de Vendas)', async ({ page }) => {
        await page.waitForTimeout(1000);
    ...
    test('R07 - Deve exibir tabela de últimos pedidos (5 itens)', async ({ page }) => {
        await page.waitForTimeout(1000);
        ...
        if (!isBlocked) {
            ...
            const ordersTable = page.locator('table, [class*="order"], [class*="list"]');
    ...
    test('R08 - Deve ter botão "Exportar" extrato de caixa', async ({ page }) => {
        ...
        if (!isBlocked) {
            // Ir para aba Caixa
            const cashTab = page.locator('button').filter({ hasText: /caixa/i }).first();
            if (await cashTab.count() > 0) await cashTab.click();
            await page.waitForTimeout(500);

            // Procurar botão exportar
            const exportButton = page.locator('button').filter({ hasText: /exportar/i }).first();
    ...
    test('R10 - Deve exibir Top 10 produtos por receita', async ({ page }) => {
        ...
        if (!isBlocked) {
            const productsTab = page.locator('button').filter({ hasText: /produtos/i }).first();
            if (await productsTab.count() > 0) await productsTab.click();
            await page.waitForTimeout(500);

            // Procurar tabela ou lista de produtos
            const productsTable = page.locator('table, [class*="product"], [class*="list"]');
...
test.describe('Creative Studio - Estúdio Criativo IA', () => {
    ...
    test('EC01 - Deve exibir 4 botões de Quick Actions', async ({ page }) => {
        ...
        const quickActions = page.locator('button').filter({ hasText: /dia.*chuvoso|vendas.*baixas|feriado|ticket/i });
    ...
    test('EC02 - Deve gerar campanha "Dia Chuvoso"', async ({ page }) => {
        ...
        const rainyButton = page.locator('button').filter({ hasText: /chuvoso|chuva/i }).first();
    ...
    test('EC03 - Deve gerar campanha "Vendas Baixas"', async ({ page }) => {
        ...
        const slowSalesButton = page.locator('button').filter({ hasText: /vendas.*baixas|movimento.*fraco/i }).first();
    ...
    test('EC04 - Deve editar título da campanha', async ({ page }) => {
        ...
        const quickAction = page.locator('button').filter({ hasText: /chuvoso|vendas|feriado/i }).first();
    ...
    test('EC05 - Deve editar copy (texto do anúncio)', async ({ page }) => {
        ...
        const quickAction = page.locator('button').filter({ hasText: /chuvoso/i }).first();
        if (await quickAction.count() > 0) await quickAction.click();
        await page.waitForTimeout(1000);

        // Procurar textarea de copy
        const copyTextarea = page.locator('textarea[placeholder*="texto"], textarea[name*="copy"]').first();
    ...
    test('EC06 - Deve ter aba "2. Impulsionamento"', async ({ page }) => {
        ...
        const nextButton = page.locator('button').filter({ hasText: /próximo|avançar|público/i }).first();
    ...
    test('EC07 - Deve ajustar raio de alcance (slider 1-10km)', async ({ page }) => {
        await page.waitForTimeout(1000);
        ...
        const nextButton = page.locator('button').filter({ hasText: /próximo|público/i }).first();
    ...
    test('EC08 - Deve ter opção "Pacote Blitz" (orçamento fixo)', async ({ page }) => {
        await page.waitForTimeout(1000);
        ...
        const nextButton = page.locator('button').filter({ hasText: /próximo/i }).first();
        if (await nextButton.count() > 0) await nextButton.click();
        await page.waitForTimeout(500);

        // Procurar opção de pacote
        const blitzOption = page.locator('button, [role="radio"]').filter({ hasText: /blitz|pacote.*fixo/i }).first();
    ...
    test('EC09 - Deve ter opção "Orçamento Livre" (CPM)', async ({ page }) => {
        ...
        const freeOption = page.locator('button, [role="radio"]').filter({ hasText: /livre|cpm/i }).first();
    ...
    test('EC10 - Deve criar campanha com botão "Criar Campanha"', async ({ page }) => {
        await page.waitForTimeout(1000);
        ...
        const createButton = page.locator('button').filter({ hasText: /criar.*campanha|lançar|publicar/i }).first();
        ...
        if (await createButton.count() > 0) {
            ...
            // Toast de sucesso
            await expect(page.locator('text=/sucesso|campanha.*criada/i')).toBeVisible({ timeout: 3000 });
        }
    });
