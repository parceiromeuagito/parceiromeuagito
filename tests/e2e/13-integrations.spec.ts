...
test.describe("Integrations - Integrações com Marketplaces", () => {
  test.beforeEach(async ({ page }) => {
    ...
    const integrationsTab = page
      ...
      .filter({ hasText: /integraç|integration/i })
      .first();
  ...
  test("INT01 - Deve exibir cards de integrações disponíveis", async ({
    page,
  }) => {
    await page.waitForTimeout(1000);
    ...
    const integrationCards = page.locator(
      '[class*="integration"], [data-integration]',
    );
  ...
  test("INT02 - Deve exibir logos dos marketplaces", async ({ page }) => {
    ...
    const logos = page.locator(
      'img[alt*="iFood"], img[alt*="Rappi"], img[alt*="Uber"], svg',
    );
    const count = await logos.count();
    expect(count).toBeGreaterThanOrEqual(0);
  });

  test("INT03 - Deve exibir status de conexão (conectado/desconectado)", async ({
    page,
  }) => {
    await page.waitForTimeout(1000);
    ...
    const statusBadges = page
      .locator('[class*="badge"]')
      .filter({ hasText: /conectado|desconectado|ativo|inativo/i });
  ...
  test("INT04 - Deve exibir data da última sincronização", async ({ page }) => {
    ...
    const timestamps = page.locator(
      "text=/última.*sinc|sinc.*há.*\d+|nunca.*sincronizado/i",
    );
  ...
  test('INT05 - iFood: Deve ter botão "Conectar"', async ({ page }) => {
    ...
    const ifoodCard = page
      .locator('[class*="integration"]')
      .filter({ hasText: /iFood/i })
      .first();
  ...
  test("INT06 - iFood: Deve solicitar API Key ao conectar", async ({
    page,
  }) => {
    await page.waitForTimeout(1000);
    ...
    if ((await connectButton.count()) > 0) {
      ...
      const apiKeyInput = page.locator(
        'input[placeholder*="API"], input[name*="api"], input[placeholder*="chave"]',
      );
    ...
    if ((await connectButton.count()) > 0) {
      ...
      if ((await apiKeyInput.count()) > 0) {
        ...
        const submitButton = page
          .locator("button")
          .filter({ hasText: /conectar|salvar/i })
          .first();
        if ((await submitButton.count()) > 0) {
          ...
          const errorMessage = page.locator(
            "text=/inválido|formato|incorreto/i",
          );
  ...
  test("INT08 - iFood: Deve confirmar conexão bem-sucedida", async ({
    page,
  }) => {
    await page.waitForTimeout(1000);
    ...
    if ((await connectButton.count()) > 0) {
      ...
      if ((await apiKeyInput.count()) > 0) {
        ...
        if ((await submitButton.count()) > 0) {
          ...
          // Toast de sucesso
          await expect(page.locator("text=/conectado|sucesso/i")).toBeVisible({
            timeout: 3000,
          });
        }
      }
  ...
  test('INT09 - Deve ter botão "Sincronizar Cardápio"', async ({ page }) => {
    ...
    const syncButton = page
      .locator("button")
      .filter({ hasText: /sincronizar.*cardápio|sync.*menu/i })
      .first();
  ...
  test("INT10 - Deve sincronizar cardápio manualmente", async ({ page }) => {
    ...
    if ((await syncButton.count()) > 0) {
      ...
      const syncToast = page.locator(
        "text=/sincronizando|sincronizado|atualizado/i",
      );
  ...
  test("INT12 - Deve ter toggle de sincronização automática", async ({
    page,
  }) => {
    await page.waitForTimeout(1000);
    ...
    const autoSyncToggle = page
      ...
      .or(page.locator("label").filter({ hasText: /automática|auto.*sync/i }));
  ...
  test("INT14 - Deve identificar origem do pedido (badge)", async ({
    page,
  }) => {
    await page.goto("/dashboard/orders");
    await page.waitForTimeout(1000);
    ...
    const sourceBadges = page
      .locator('[class*="badge"], [class*="source"]')
      .filter({ hasText: /iFood|Rappi|Online|Balcão/i });
    const count = await sourceBadges.count();
    expect(count).toBeGreaterThanOrEqual(0);
  });

  test("INT15 - Deve aceitar pedidos de marketplace normalmente", async ({
    page,
  }) => {
    await page.goto("/dashboard/orders");
    await page.waitForTimeout(1000);
  ...
  test("INT16 - Deve enviar atualização de status para marketplace", async ({
    page,
  }) => {
    await page.goto("/dashboard/orders");
    await page.waitForTimeout(1000);
  ...
  test('INT17 - Deve ter botão "Desconectar" em integrações ativas', async ({
    page,
  }) => {
    await page.waitForTimeout(1000);
    ...
    const disconnectButton = page
      .locator("button")
      .filter({ hasText: /desconectar|disconnect/i })
      .first();
  ...
  test("INT18 - Deve confirmar antes de desconectar", async ({ page }) => {
    ...
    if ((await disconnectButton.count()) > 0) {
      ...
      const confirmText = page.locator(
        "text=/tem certeza|confirmar|desconectar/i",
      );
  ...
  test("INT19 - Deve desconectar integração", async ({ page }) => {
    ...
    if ((await disconnectButton.count()) > 0) {
      ...
      const confirmButton = page
        .locator("button")
        .filter({ hasText: /sim|confirmar|desconectar/i })
        .first();
      if ((await confirmButton.count()) > 0) {
        ...
        // Toast de desconexão
        await expect(page.locator("text=/desconectado|removido/i")).toBeVisible(
          { timeout: 3000 },
        );
      }
    }
  ...
  test("INT20 - Deve bloquear integrações no plano Starter", async ({
    page,
  }) => {
    ...
    if (isStarter) {
      ...
      const upgradeScreen = page.locator(
        "text=/upgrade|Pro|Enterprise|plano/i",
      );
...
test.describe("Webhooks - Callbacks e Eventos", () => {
  ...
  test('WH02 - Deve ter botão "Copiar URL"', async ({ page }) => {
    ...
    const copyButton = page
      .locator('button[aria-label*="copiar"], button')
      .filter({ hasText: /copiar/i })
      .first();
  ...
  test("WH03 - Deve testar webhook", async ({ page }) => {
    ...
    const testButton = page
      .locator("button")
      .filter({ hasText: /testar.*webhook|test/i })
      .first();
  ...
  test("WH04 - Deve exibir log de webhooks recebidos", async ({ page }) => {
    ...
    const webhookLog = page.locator(
      "text=/log|histórico.*webhook|eventos.*recebidos/i",
    );
