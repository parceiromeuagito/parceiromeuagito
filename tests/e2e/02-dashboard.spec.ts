...
test.describe('Dashboard - Painel Principal', () => {
    ...
    test('D01 - Deve mostrar skeleton durante carregamento', async ({ page }) => {
        ...
        const skeleton = page.locator('[data-testid="skeleton"], .animate-pulse, [role="status"]');
    ...
    test('D02 - Deve exibir 4 cards de estatísticas', async ({ page }) => {
        ...
        const statCards = page.locator('[class*="stat"], [class*="card"]').filter({ hasText: /Receita|Pedidos|Andamento|Concluídos/i });
    ...
    test('D04 - Deve renderizar gráfico de receita', async ({ page }) => {
        ...
        const chart = page.locator('canvas, svg').filter({ has: page.locator('[class*="echarts"]') });
    ...
    test('D05 - Deve ter botões de seleção de período', async ({ page }) => {
        ...
        const periodButtons = page.locator('button').filter({ hasText: /Dia|Semana|Mês/i });
    ...
    test('D06 - Deve ter botão "Exportar CSV"', async ({ page }) => {
        const exportButton = page.locator('button').filter({ hasText: /Exportar.*CSV/i });
        ...
        await expect(page.locator('text=/gerado|sucesso/i')).toBeVisible({ timeout: 3000 });
    });

    // ========================================
    // D07: Botão PDF Detalhado (Pro)
    // ========================================
    test('D07 - Deve ter botão "PDF Detalhado" se plano Pro/Enterprise', async ({ page }) => {
        const pdfButton = page.locator('button').filter({ hasText: /PDF.*Detalhado/i });
    ...
    // ========================================
    test('D09 - Deve exibir card de insights IA (se Enterprise)', async ({ page }) => {
        await page.waitForTimeout(2000);
    ...
    test('D11 - Deve exibir feed de atividades', async ({ page }) => {
        ...
        const activityFeed = page.locator('text=/atividade|recente|último/i, [class*="activity"], [class*="feed"]');
    ...
    test('D12 - Deve exibir badges de tendência nos cards', async ({ page }) => {
        ...
        const trendElements = page.locator('[class*="text-green"], [class*="text-red"], [class*="trend"]');
