...
test.describe("Roles & Permissions - Controle de Acesso", () => {
  ...
  test("ROLE06 - Manager: Deve ver menu sem Configurações e Creative", async ({
    page,
  }) => {
  ...
  test("ROLE08 - Manager: Deve ser redirecionado ao acessar Settings", async ({
    page,
  }) => {
    await loginWithRole(page, "manager");
  ...
  test("ROLE10 - Cashier: Deve ver menu limitado (Pedidos, POS, Catálogo, Mensagens)", async ({
    page,
  }) => {
  ...
  test("ROLE15 - Cashier: Deve ver catálogo (apenas visualização)", async ({
    page,
  }) => {
    await loginWithRole(page, "cashier");
