...
test.describe("Order Types - Tipos Específicos de Pedido", () => {
  ...
  test("OT01 - Delivery: Deve exibir endereço de entrega", async ({ page }) => {
    ...
    const deliveryOrders = page
      .locator('[data-type="delivery"], [class*="delivery"]')
      .first();
  ...
  test("OT02 - Delivery: Deve exibir tempo estimado de entrega", async ({
    page,
  }) => {
    await page.waitForTimeout(1000);
  ...
  test('OT03 - Delivery: Deve ter status "Saiu para entrega"', async ({
    page,
  }) => {
    await page.waitForTimeout(1000);

    // Procurar status específico de delivery
    const deliveringStatus = page.locator(
      "text=/saiu.*entrega|em rota|delivering/i",
    );
  ...
  test('OT04 - Pickup: Deve exibir badge "Retirada"', async ({ page }) => {
    ...
    const pickupBadge = page
      .locator('[class*="badge"]')
      .filter({ hasText: /retirada|pickup/i });
  ...
  test('OT06 - Pickup: Deve ter status "Pronto para retirada"', async ({
    page,
  }) => {
    await page.waitForTimeout(1000);
  ...
  test('OT09 - Table: Deve ter botão "Abrir Comanda"', async ({ page }) => {
    ...
    if ((await tableOrders.count()) > 0) {
      ...
      // Botão de comanda
      const comandaButton = page
        .locator("button")
        .filter({ hasText: /comanda|conta/i })
        .first();
  ...
  test("OT10 - Booking: Deve exibir data e hora da reserva", async ({
    page,
  }) => {
    await page.waitForTimeout(1000);
  ...
  test('OT12 - Booking: Deve ter status "Confirmado"', async ({ page }) => {
    ...
    // Status de reserva
    const confirmedStatus = page
      .locator('[class*="badge"]')
      .filter({ hasText: /confirmad|confirmed/i });
  ...
  test("OT15 - Event: Deve exibir tipo de ingresso (VIP, Pista, etc)", async ({
    page,
  }) => {
    await page.waitForTimeout(1000);
  ...
  test("OT16 - Stay: Deve exibir datas de check-in e check-out", async ({
    page,
  }) => {
    await page.waitForTimeout(1000);
    ...
    if ((await stayOrders.count()) > 0) {
      ...
      const checkInOut = page.locator(
        "text=/check-in|check-out|\d{2}\/\d{2}/i",
      );
...
test.describe("Order Type Filtering - Filtros por Tipo", () => {
  ...
  test('FILTER03 - Deve mostrar todos os tipos quando "Todos" selecionado', async ({
    page,
  }) => {
    const typeFilter = page
      .locator("select")
