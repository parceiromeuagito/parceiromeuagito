...
test.describe('Chat - Comunicação com Clientes', () => {
    ...
    test('CH01 - Deve exibir lista de conversas na sidebar', async ({ page }) => {
        ...
        const conversations = page.locator('[class*="conversation-item"], [data-conversation]');
        const count = await conversations.count();
        expect(count).toBeGreaterThanOrEqual(0);
    });

    test('CH02 - Deve exibir nome do cliente em cada conversa', async ({ page }) => {
        await page.waitForTimeout(1000);
    ...
    test('CH03 - Deve exibir badge de mensagens não lidas', async ({ page }) => {
        ...
        const unreadBadges = page.locator('[class*="badge"], [class*="unread"]').filter({ hasText: /\d+/ });
    ...
    test('CH04 - Deve exibir prévia da última mensagem', async ({ page }) => {
        ...
        if (await conversationItems.count() > 0) {
            const preview = page.locator('[class*="preview"], [class*="last-message"]');
    ...
    test('CH05 - Deve exibir timestamp da última mensagem', async ({ page }) => {
        ...
        const timestamps = page.locator('text=/há.*\d+.*min|há.*\d+.*hora|\d{2}:\d{2}|Ontem|Hoje/i');
    ...
    test('CH06 - Deve selecionar conversa ao clicar', async ({ page }) => {
        ...
        const firstConversation = page.locator('[class*="conversation-item"]').first();
        ...
        if (await firstConversation.count() > 0) {
            ...
            const chatArea = page.locator('[class*="chat-area"], [class*="messages"]');
    ...
    test('CH08 - Deve diferenciar mensagens enviadas e recebidas', async ({ page }) => {
        await page.waitForTimeout(1000);
        ...
        if (await firstConversation.count() > 0) {
            ...
            const receivedMessages = page.locator('[class*="received"], [class*="left"]');
    ...
    test('CH09 - Deve enviar mensagem ao pressionar Enter', async ({ page }) => {
        ...
        if (await firstConversation.count() > 0) {
            ...
            const messageInput = page.locator('textarea, input[placeholder*="mensagem"], input[placeholder*="Digite"]').first();
    ...
    test('CH10 - Deve enviar mensagem ao clicar no botão enviar', async ({ page }) => {
        await page.waitForTimeout(1000);
        ...
        if (await firstConversation.count() > 0) {
            ...
            const messageInput = page.locator('textarea, input[placeholder*="mensagem"]').first();
            const sendButton = page.locator('button[aria-label*="enviar"], button').filter({ has: page.locator('svg') }).last();
    ...
    test('CH11 - Deve exibir informações do pedido no cabeçalho', async ({ page }) => {
        await page.waitForTimeout(1000);
        ...
        if (await firstConversation.count() > 0) {
            ...
            const orderInfo = page.locator('[class*="order-info"], [class*="header"]');
    ...
    test('CH12 - Deve ter botão para ver detalhes do pedido', async ({ page }) => {
        await page.waitForTimeout(1000);
        ...
        if (await firstConversation.count() > 0) {
            ...
            const detailsButton = page.locator('button').filter({ hasText: /ver.*pedido|detalhes/i }).first();
    ...
    test('CH13 - Deve filtrar conversas por status (Todas/Não Lidas)', async ({ page }) => {
        await page.waitForTimeout(1000);

        // Procurar filtros
        const filterButtons = page.locator('button').filter({ hasText: /todas|não.*lidas|ativas/i });
    ...
    test('CH15 - Deve exibir empty state quando não há conversas', async ({ page }) => {
        await page.waitForTimeout(1000);
