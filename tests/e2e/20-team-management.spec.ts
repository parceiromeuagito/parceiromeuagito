...
test.describe("Team Management - Gestão de Equipe", () => {
  ...
  test("TM03 - Deve exibir status do membro (ativo/inativo)", async ({
    page,
  }) => {
    await page.waitForTimeout(500);
  ...
  test('TM04 - Deve ter botão "Adicionar Membro"', async ({ page }) => {
    ...
    const addBtn = page
      .locator("button")
      .filter({ hasText: /adicionar.*membro|novo.*membro/i })
      .first();
  ...
  test("TM11 - Deve confirmar antes de remover", async ({ page }) => {
    ...
    if ((await removeBtn.count()) > 0) {
      ...
      const confirmDialog = page.locator(
        '[role="dialog"], text=/confirmar|tem certeza/i',
      );
