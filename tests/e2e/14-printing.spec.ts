...
test.describe('Printing - Sistema de Impressão', () => {
    ...
    test('PR02 - Deve selecionar tamanho de papel (58mm)', async ({ page }) => {
        ...
        // Selecionar 58mm
        const paper58mm = page.locator('button').filter({ hasText: /58mm/i }).first();
    ...
    test('PR03 - Deve selecionar tamanho de papel (80mm)', async ({ page }) => {
        ...
        // Selecionar 80mm
        const paper80mm = page.locator('button').filter({ hasText: /80mm/i }).first();
    ...
    test('PR06 - Deve ativar impressão automática', async ({ page }) => {
        ...
        const autoPrintToggle = page.locator('input[type="checkbox"]').filter({ hasText: /auto/i }).or(
            page.locator('label').filter({ hasText: /auto.*imprimir/i }).locator('input')
        ).first();
    ...
    test('PR08 - Deve abrir preview ao clicar em "Imprimir Teste"', async ({ page }) => {
        await page.goto('/dashboard/settings');
        await page.waitForTimeout(1000);
        ...
        const testButton = page.locator('button').filter({ hasText: /imprimir.*teste/i }).first();
    ...
    test('PR09 - Preview deve mostrar formatação correta', async ({ page }) => {
        ...
        if (await testButton.count() > 0) {
            ...
            const previewContent = page.locator('[class*="preview"], [class*="receipt"]');
    ...
    test('PR10 - Deve imprimir cupom ao finalizar venda no POS', async ({ page }) => {
    ...
    test('PR11 - Deve ter botão "Reimprimir Cupom" após venda', async ({ page }) => {
        await page.goto('/dashboard/pos');
        await page.waitForTimeout(1000);

        // Procurar botão de reimprimir
        const reprintButton = page.locator('button').filter({ hasText: /reimprimir/i }).first();
    ...
    test('PR12 - Deve selecionar número de vias (1-3)', async ({ page }) => {
        ...
        const copiesSelect = page.locator('select, [role="combobox"]').filter({ hasText: /cópia|via/i }).first();
    ...
    test('PR13 - Deve imprimir cupom ao aceitar pedido (se auto-print ativo)', async ({ page }) => {
    ...
    test('PR14 - Deve ter botão "Imprimir" no detalhes do pedido', async ({ page }) => {
        await page.goto('/dashboard/orders');
        await page.waitForTimeout(1000);
        ...
        if (await orderCard.count() > 0) {
            ...
            const printButton = page.locator('button').filter({ hasText: /imprimir/i }).first();
            const count = await printButton.count();
            expect(count).toBeGreaterThanOrEqual(0);
        }
    });

    test('PR15 - Cupom deve incluir todos os dados do pedido', async ({ page }) => {
        await page.goto('/dashboard/orders');
        await page.waitForTimeout(1000);
