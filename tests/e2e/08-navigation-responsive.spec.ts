...
test.describe("Navigation - Sidebar e Header", () => {
  ...
  test("N01 - Deve navegar para Dashboard ao clicar no link", async ({
    page,
  }) => {
    ...
    const dashboardLink = page
      ...
      .filter({ hasText: /dashboard|painel|início/i })
      .first();
  ...
  test("N04 - Deve expandir sidebar ao clicar no chevron novamente", async ({
    page,
  }) => {
    await page.waitForTimeout(1000);
  ...
  test("N05 - Deve exibir badge verde quando caixa está aberto", async ({
    page,
  }) => {
    await page.waitForTimeout(1000);
  ...
  test("N06 - Deve exibir badge vermelho quando caixa está fechado", async ({
    page,
  }) => {
    await page.waitForTimeout(1000);
  ...
  test("N07 - Deve abrir modal de caixa ao clicar no botão", async ({
    page,
  }) => {
    await page.waitForTimeout(1000);
...
test.describe("Responsive - Adaptação de Layout", () => {
  ...
  test("RES01 - Deve adaptar layout para mobile (< 768px)", async ({
    page,
  }) => {
  ...
  test("RES02 - Deve adaptar layout para tablet (768-1024px)", async ({
    page,
  }) => {
  ...
  test("RES03 - Deve adaptar layout para desktop (> 1024px)", async ({
    page,
  }) => {
  ...
  test("RES05 - Deve ter overlay ao abrir sidebar no mobile", async ({
    page,
  }) => {
    // Mobile
    await page.setViewportSize({ width: 375, height: 667 });
...
test.describe("Extras - Funcionalidades Gerais", () => {
  ...
  test("EXTRA02 - Deve exibir nome do usuário no header", async ({ page }) => {
    ...
    const userDisplay = page
      .locator('[class*="user"], [class*="profile"]')
      .first();
