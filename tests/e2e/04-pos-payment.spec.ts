...
test.describe('POS - Frente de Caixa', () => {
    ...
    test('P02 - Deve focar no campo de busca ao pressionar F2', async ({ page }) => {
        // Pressionar F2
        await page.keyboard.press('F2');

        // Campo de busca deve estar focado
        const searchInput = page.locator('input[type="search"], input#pos-search-input, input[placeholder*="busca"]');
        await expect(searchInput.first()).toBeFocused({ timeout: 2000 });
    });

    test('P03 - Deve buscar produto ao digitar', async ({ page }) => {
        const searchInput = page.locator('input[type="search"], input#pos-search-input, input[placeholder*="busca"]').first();
    ...
    test('P04 - Deve filtrar por categoria', async ({ page }) => {
        ...
        const categoryButtons = page.locator('button').filter({ hasText: /categoria|bebida|comida|todos/i });
    ...
    test('P05 - Deve adicionar produto ao carrinho ao clicar', async ({ page }) => {
        const productCard = page.locator('[class*="product-card"], button[data-product]').first();
    ...
    test('P06 - Deve aumentar quantidade com botão +', async ({ page }) => {
        ...
        if (await increaseButton.count() > 0) {
            const initialQty = await page.locator('text=/\\d+x|Qtd.*\\d+/i').first().textContent();
            await increaseButton.click();
            await page.waitForTimeout(300);

            // Quantidade deve aumentar
            const newQty = await page.locator('text=/\\d+x|Qtd.*\\d+/i').first().textContent();
    ...
    test('P08 - Deve remover item ao diminuir quantidade até 0', async ({ page }) => {
        const productCard = page.locator('[class*="product-card"]').first();
        if (await productCard.count() > 0) await productCard.click();
        await page.waitForTimeout(300);
    ...
    test('P09 - Deve limpar carrinho ao clicar no ícone lixeira', async ({ page }) => {
        // Adicionar produto
        const productCard = page.locator('[class*="product-card"]').first();
        if (await productCard.count() > 0) await productCard.click();
        await page.waitForTimeout(300);
        ...
        const clearButton = page.locator('button[aria-label*="limpar"], button').filter({ has: page.locator('svg[class*="trash"]') }).first();
    ...
    test('P10 - Deve calcular subtotal corretamente', async ({ page }) => {
        ...
        // Verificar subtotal
        const subtotal = page.locator('text=/subtotal|sub-total/i').locator('..').locator('text=/R\\$/');
    ...
    test('P11 - Deve aplicar desconto corretamente', async ({ page }) => {
        ...
        const discountInput = page.locator('input[placeholder*="desconto"], input[name*="discount"]').first();
        ...
        if (await discountInput.count() > 0) {
            ...
            const total = page.locator('text=/total/i').locator('..').locator('text=/R\\$/');
    ...
    test('P12 - Deve calcular total com desconto', async ({ page }) => {
        ...
        // Verificar total
        const total = page.locator('text=/total.*pagar|total/i').locator('..').locator('text=/R\\$/');
        await expect(total.first()).toBeVisible();
    });

    test('P13 - Deve abrir modal de pagamento ao pressionar F9', async ({ page }) => {
        // Adicionar produto primeiro
        const productCard = page.locator('[class*="product-card"]').first();
        if (await productCard.count() > 0) await productCard.click();
        await page.waitForTimeout(300);
    ...
    test('P14 - Deve abrir modal ao clicar "Finalizar Venda"', async ({ page }) => {
        // Adicionar produto
        const productCard = page.locator('[class*="product-card"]').first();
        if (await productCard.count() > 0) await productCard.click();
        await page.waitForTimeout(300);
        ...
        // Clicar finalizar
        const finalizeButton = page.locator('button').filter({ hasText: /finalizar.*venda|F9/i }).first();
    ...
    test('P15 - Deve bloquear venda se caixa fechado', async ({ page }) => {
        ...
        const closedBadge = page.locator('[class*="red"], [class*="closed"]').filter({ hasText: /fechado/i });
        ...
        if (await closedBadge.count() > 0) {
            ...
            // Tentar finalizar
            const finalizeButton = page.locator('button').filter({ hasText: /finalizar/i }).first();
            if (await finalizeButton.count() > 0) {
                await finalizeButton.click();

                // Deve mostrar toast de erro
                await expect(page.locator('text=/caixa.*fechado/i')).toBeVisible({ timeout: 3000 });
            }
        }
    });
    ...
    test('P16 - Deve bloquear se carrinho vazio', async ({ page }) => {
        ...
        if (await finalizeButton.count() > 0) {
            ...
            if (!isDisabled) {
                await finalizeButton.click();
                await expect(page.locator('text=/carrinho.*vazio|adicione.*produtos/i')).toBeVisible({ timeout: 3000 });
            }
        }
    });
    ...
    test('P18 - Deve confirmar antes de limpar venda com ESC', async ({ page }) => {
        // Adicionar produto
        const productCard = page.locator('[class*="product-card"]').first();
        if (await productCard.count() > 0) await productCard.click();
        await page.waitForTimeout(300);
    ...
    test('P19 - Deve exibir nome do cliente ou "Consumidor Final"', async ({ page }) => {
    ...
    test('PM01 - Modal de pagamento: Selecionar Dinheiro', async ({ page }) => {
        ...
        // Selecionar dinheiro
        const cashButton = page.locator('button').filter({ hasText: /dinheiro|cash/i }).first();

        if (await cashButton.count() > 0) {
            await cashButton.click();

            // Campos de troco devem aparecer
            const receivedInput = page.locator('input[placeholder*="recebido"], input[name*="received"]');
            await expect(receivedInput.first()).toBeVisible({ timeout: 2000 });
        }
    });

    test('PM02 - Modal de pagamento: Selecionar Cartão Crédito', async ({ page }) => {
        const productCard = page.locator('[class*="product-card"]').first();
        if (await productCard.count() > 0) await productCard.click();
        await page.waitForTimeout(300);
        await page.keyboard.press('F9');
        ...
        const creditButton = page.locator('button').filter({ hasText: /crédito|credit/i }).first();

        if (await creditButton.count() > 0) {
            await creditButton.click();

            // Campo de parcelas deve aparecer
            const installmentsSelect = page.locator('select, [role="combobox"]').filter({ hasText: /parcela/i });
            const count = await installmentsSelect.count();
            expect(count).toBeGreaterThanOrEqual(0);
        }
    });

    test('PM03 - Modal de pagamento: Selecionar Cartão Débito', async ({ page }) => {
        const productCard = page.locator('[class*="product-card"]').first();
        if (await productCard.count() > 0) await productCard.click();
        await page.waitForTimeout(300);
        await page.keyboard.press('F9');
        ...
        const debitButton = page.locator('button').filter({ hasText: /débito|debit/i }).first();
    ...
    test('PM04 - Modal de pagamento: Selecionar PIX', async ({ page }) => {
        ...
        const pixButton = page.locator('button').filter({ hasText: /pix/i }).first();
    ...
    test('PM05 - Modal de pagamento: Calcular troco', async ({ page }) => {
        ...
        // Selecionar dinheiro
        const cashButton = page.locator('button').filter({ hasText: /dinheiro/i }).first();
        if (await cashButton.count() > 0) await cashButton.click();

        // Inserir valor recebido
        const receivedInput = page.locator('input[placeholder*="recebido"]').first();
    ...
    test('PM06 - Modal de pagamento: Confirmar pagamento completo', async ({ page }) => {
        const productCard = page.locator('[class*="product-card"]').first();
        if (await productCard.count() > 0) await productCard.click();
        await page.waitForTimeout(300);
        await page.keyboard.press('F9');
        ...
        // Confirmar
        const confirmButton = page.locator('button').filter({ hasText: /confirmar|finalizar/i }).first();
        if (await confirmButton.count() > 0) {
            await confirmButton.click();
            await page.waitForTimeout(1000);

            // Deve mostrar toast de sucesso
            await expect(page.locator('text=/sucesso|venda.*realizada/i')).toBeVisible({ timeout: 3000 });
        }
    });
    ...
    test('PM07 - Modal de pagamento: Cancelar pagamento', async ({ page }) => {
        ...
        // Fechar modal
        const closeButton = page.locator('button[aria-label*="fechar"], button').filter({ has: page.locator('svg') }).first();
    ...
    test('PM09 - Modal de pagamento: Selecionar parcelas 1-12x', async ({ page }) => {
        const productCard = page.locator('[class*="product-card"]').first();
        if (await productCard.count() > 0) await productCard.click();
        await page.waitForTimeout(300);
        await page.keyboard.press('F9');
        ...
        // Selecionar crédito
        const creditButton = page.locator('button').filter({ hasText: /crédito/i }).first();
    ...
    test('PM10 - Modal de pagamento: Validar valor menor que total', async ({ page }) => {
        const productCard = page.locator('[class*="product-card"]').first();
        if (await productCard.count() > 0) await productCard.click();
        await page.waitForTimeout(300);
        await page.keyboard.press('F9');
    ...
    test('CX01 - Deve abrir modal de caixa', async ({ page }) => {
        ...
        const cashButton = page.locator('button').filter({ hasText: /caixa|cash.*register/i }).first();
    ...
    test('CX02 - Deve abrir caixa com valor inicial', async ({ page }) => {
        ...
        const openButton = page.locator('button').filter({ hasText: /abrir.*caixa/i }).first();

        if (await openButton.count() > 0) {
            // Inserir valor inicial
            const amountInput = page.locator('input[type="number"], input[placeholder*="valor"]').first();
            ...
            // Toast de sucesso
            await expect(page.locator('text=/caixa.*aberto|aberto.*sucesso/i')).toBeVisible({ timeout: 3000 });
        }
    });
    ...
    test('CX03 - Deve validar abertura sem valor', async ({ page }) => {
        ...
        const openButton = page.locator('button').filter({ hasText: /abrir/i }).first();
        ...
        if (await openButton.count() > 0) {
            ...
            if (!isDisabled) {
                await openButton.click();
                await expect(page.locator('text=/valor.*válido|preencha/i')).toBeVisible({ timeout: 3000 });
            }
        }
    });
    ...
    test('CX04 - Deve realizar suprimento (entrada)', async ({ page }) => {
        ...
        const supplyButton = page.locator('button').filter({ hasText: /suprimento|entrada/i }).first();
        ...
        if (await supplyButton.count() > 0) {
            ...
            if (await amountInput.count() > 0) {
                ...
                // Confirmar
                const confirmBtn = page.locator('button').filter({ hasText: /confirmar/i }).first();
    ...
    test('CX05 - Deve realizar sangria (saída)', async ({ page }) => {
        ...
        const bleedButton = page.locator('button').filter({ hasText: /sangria|retirada/i }).first();
    ...
    test('CX06 - Deve validar sangria maior que saldo', async ({ page }) => {
        ...
        const bleedButton = page.locator('button').filter({ hasText: /sangria/i }).first();
        ...
        if (await bleedButton.count() > 0) {
            ...
            if (await amountInput.count() > 0) {
                ...
                if (await confirmBtn.count() > 0) {
                    ...
                    // Deve mostrar erro
                    await expect(page.locator('text=/saldo.*insuficiente|valor.*inválido/i')).toBeVisible({ timeout: 3000 });
                }
            }
        }
    });
    ...
    test('CX07 - Deve fechar caixa', async ({ page }) => {
        ...
        const closeButton = page.locator('button').filter({ hasText: /fechar.*caixa/i }).first();
    ...
    test('CX08 - Deve exibir histórico de transações', async ({ page }) => {
        ...
        const transactionList = page.locator('table, [class*="transaction"], [class*="history"]');
